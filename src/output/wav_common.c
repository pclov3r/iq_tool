...
bool output_wav_common_initialize(ModuleContext *context, int sf_format_flag) {
  ...
  WavCommonContext *data = (WavCommonContext *)mem_arena_alloc(
      &app->process_chain.setup_arena, sizeof(WavCommonContext), true);
  if (!data)
    return false;
...
// Use platform-specific UTF-8 path for messages.
  ...
  if (!sf_format_check(&sfinfo)) {
    log_error("libsndfile does not support the requested format (Rate: %d, "
              "Format: 0x%08X).",
...
// Open the file using the appropriate platform-specific function.
  ...
  size_t bps = (size_t)app->dsp.process_chain_sample_rate_hz *
               app->module.output_bytes_per_iq_sample * 2;
  int bits = (int)app->module.output_bytes_per_iq_sample * 8;
  const char *radio_model =
      config->input.type_name ? config->input.type_name : "iq_tool";

#ifndef GIT_HASH
#define GIT_HASH "unknown"
#endif

  // Construct the strictly formatted filename exclusively for the XML
  // attributes to satisfy SDR Console's regex parser
  char sdr_console_xml_filename[256];
  snprintf(sdr_console_xml_filename, sizeof(sdr_console_xml_filename),
           "%s %02d%02d%02d.000 %.3fMHz.wav", date_only,
  ...
  char true_title[256];
  snprintf(true_title, sizeof(true_title),
           "%.3f MHz, BW %.0f kHz, %04d-%02d-%02d %02d:%02d",
           config->sdr_general.rf_freq_hz / 1000000.0,
           app->dsp.process_chain_sample_rate_hz / 1000.0,
  ...
  int xml_length = snprintf(
      ...
      "UTCSeconds=\"%lld\"/>"
      "</SDR-XML-Root>",
      time_created, time_utc, sdr_console_xml_filename,
      sdr_console_xml_filename, radio_model, GIT_HASH, time_utc, bits, bps,
  ...
  if (xml_length > 0 && xml_length < (int)sizeof(xml_buffer)) {
    ...
    if (utf16_buffer) {
      ...
      if (set_res != SF_ERR_NO_ERROR) {
        log_warn("Failed to write XML metadata chunk: %s",
                 sf_error_number(set_res));
      }
    }
  }

  // Fallback: Also write standard WAV RIFF INFO tags!
  // SDR Console (and other audio players) often read Title and Artist from here
  // instead of the custom XML. Use true_title so it perfectly matches the
  // native SDR Console UI format
...
size_t output_wav_common_write_chunk(ModuleContext *context, const void *buffer,
                                     size_t bytes_to_write) {
  AppContext *app = context->app;
  WavCommonContext *data = (WavCommonContext *)app->module.output_private_data;
  if (!data || !data->handle || bytes_to_write == 0)
    return 0;
