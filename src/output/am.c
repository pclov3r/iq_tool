 ...
 * 1. Synchronous AM (SYNC) using a Phase-Locked Loop (PLL) for high-fidelity
 * audio.
 * 2. Envelope AM (ENV) using complex magnitude detection.
 *
 * DSP ProcessChain:
 * - Block Filtering: Uses liquid-dsp SIMD block execution for high-speed
 * channel filtering.
 ...
 * - Leveling: Audio volume is strictly normalized using an asymmetric
 * Exponential Moving Average (EMA) envelope tracker to maintain consistent
 * volume at all SNRs.
 */
...
static struct {
  ...
  bool force_envelope;
    ...
    .gain_val = 1.0f, .audio_cutoff = 5000.0f, .force_envelope = 0};
...
static bool output_am_validate_options(AppContext *app) {
  ...
  if (config->baseband_sample_rate.rate_hz == 0.0) {
    ...
    if (rate < AM_MIN_INPUT_RATE) {
      log_error("AM: Input rate %.15g Hz is too low for audio (Min: %.15g).",
                rate, AM_MIN_INPUT_RATE);
      return false;
    }
    if (rate > AM_MAX_INPUT_RATE) {
      log_error("AM: Input rate %.15g Hz is unnecessarily high (Max: %.15g).",
                rate, AM_MAX_INPUT_RATE);
      return false;
    }
  }
  return true;
...
static bool output_am_initialize(ModuleContext *context) {
  ...
  am_context->stat_threshold =
      (size_t)(input_rate * CONSOLE_UPDATE_INTERVAL_SEC);
  ...
  log_info("AM: Baseband %.15g Hz | Audio Cutoff: %.15g Hz | Mode: %s",
           ...
           am_context->sync_mode ? "Synchronous (PLL)"
                                 : "Envelope (Magnitude)");
  ...
  if (fc_baseband > 0.49f)
    fc_baseband = 0.49f;
  liquid_firdes_kaiser(h_len, fc_baseband,
                       context->config->dsp.filter.args.attenuation, 0.0f, hc);
  ...
  am_context->resamp_out = msresamp_rrrf_create(
      output_resample_ratio, context->config->dsp.filter.args.attenuation);
  ...
  if (fc > 0.49f)
    fc = 0.49f;
  liquid_firdes_kaiser(h_length, fc,
                       context->config->dsp.filter.args.attenuation, 0.0f, h);
  ...
  size_t out_buf_samples =
      (size_t)ceil(buf_samples * output_resample_ratio) + 128;

  am_context->filtered_baseband =
      mem_arena_alloc(&res->process_chain.setup_arena,
                      buf_samples * sizeof(liquid_float_complex), false);
  am_context->mono_buffer = mem_arena_alloc(&res->process_chain.setup_arena,
                                            buf_samples * sizeof(float), false);
  ...
  if (!am_context->filtered_baseband || !am_context->mono_buffer ||
      !am_context->resamp_buffer || !am_context->final_audio_buffer ||
      !am_context->interleaved_pcm)
    return false;
...
static void output_am_flush(ModuleContext *context) {
  ...
  if (is_shutdown_requested()) {
    ...
    audio_output_drain(am_context->audio_out);
  }
}
...
static size_t output_am_write_chunk(ModuleContext *context, const void *buffer,
  ...
  firfilt_crcf_execute_block(am_context->channel_filter, iq_ptr, num_frames,
                             am_context->filtered_baseband);
  ...
  for (unsigned int i = 0; i < num_frames; i++) {
    ...
    if (am_context->sync_mode) {
      if (!am_context->pll_tracking_mode &&
          am_context->pll_lock_counter++ >= am_context->pll_lock_samples) {
        nco_crcf_pll_set_bandwidth(
            am_context->pll, AM_PLL_TRACKING_BW / am_context->input_samplerate);
      ...
      am_context->carrier_i_smoothed =
          (alpha_i * prod_i) +
          ((1.0f - alpha_i) * am_context->carrier_i_smoothed);
      ...
      if ((i & 3) == 0) {
        am_context->accum_carrier_freq_sum +=
            (nco_crcf_get_frequency(am_context->pll) / (2.0f * (float)M_PI)) *
            am_context->input_samplerate;
    ...
    // Exact DC Blocker
    float audio = raw_envelope - am_context->dc_x_prev +
                  (am_context->dc_R * am_context->dc_y_prev);
  ...
  if (am_context->stat_counter >= am_context->stat_threshold) {
    float dbfs =
        utility_calculate_dbfs((float)(am_context->accum_mag_sq_sum /
                                       (double)am_context->stat_counter));

    if (am_context->sync_mode && am_context->accum_pll_count > 0) {

      float mean_i = (float)(am_context->accum_i_sum /
                             (double)am_context->accum_pll_count);
      float carrier_power = mean_i * mean_i;
      float noise_power = (float)(am_context->accum_q_sq_sum /
                                  (double)am_context->accum_pll_count);
      ...
      if (dbfs < -70.0f) {
        if (am_context->pll_is_locked) {
          ...
          nco_crcf_pll_set_bandwidth(am_context->pll,
                                     AM_PLL_FLYWHEEL_BW /
                                         am_context->input_samplerate);
        }
        am_context->lock_counter = 0;
        ...
        if (snr_db < 6.0f) {
          if (++am_context->unlock_counter >= AM_LOCK_TIMEOUT_SEC) {
            log_warn(
                "AM: PLL failed to lock. Falling back to Envelope detection.");
            am_context->pll_is_locked = false;
            am_context->lock_counter = 0;
            nco_crcf_pll_set_bandwidth(am_context->pll,
                                       AM_PLL_FLYWHEEL_BW /
        ...
        if (snr_db > 10.0f) {
          if (am_context->lock_counter == 0) {
            nco_crcf_pll_set_bandwidth(
                am_context->pll, AM_PLL_ACQ_BW / am_context->input_samplerate);
          }
      ...
      const char *state_tag =
          am_context->pll_is_locked
              ? "SYNC PLL LOCKED"
              : (am_context->lock_counter > 0 ? "ATTEMPTING PLL LOCK"
                                              : "ENVELOPE");

      log_info("dBFS: %.1f | SNR: %.1f dB | Offset: %.2f Hz [%s]", dbfs, snr_db,
               (float)(am_context->accum_carrier_freq_sum /
                       (double)am_context->accum_pll_count),
               state_tag);
  ...
  msresamp_rrrf_execute(am_context->resamp_out, am_context->mono_buffer,
                        num_frames, am_context->resamp_buffer, &num_resampled);

  // 4. SIMD Block Processing: Audio Lowpass Filter
  firfilt_rrrf_execute_block(am_context->audio_lpf, am_context->resamp_buffer,
                             num_resampled, am_context->final_audio_buffer);
  ...
  for (unsigned int i = 0; i < num_resampled; i++) {
    ...
    if (abs_sample > am_context->volume_envelope) {
      am_context->volume_envelope =
          (attack_alpha * abs_sample) +
          ((1.0f - attack_alpha) * am_context->volume_envelope);
    } else {
      am_context->volume_envelope =
          (decay_alpha * abs_sample) +
          ((1.0f - decay_alpha) * am_context->volume_envelope);
    }
...
static void output_am_cleanup(ModuleContext *context) {
  ...
  if (am_context->channel_filter)
    firfilt_crcf_destroy(am_context->channel_filter);
  if (am_context->pll)
    nco_crcf_destroy(am_context->pll);
  if (am_context->audio_lpf)
    firfilt_rrrf_destroy(am_context->audio_lpf);
  if (am_context->resamp_out)
    msresamp_rrrf_destroy(am_context->resamp_out);
}
...
static void output_am_get_summary_info(const ModuleContext *context,
  ...
  utility_add_summary_item(info, "Mode", "%s",
                           s_am_config.force_envelope ? "Envelope (Mag)"
                                                      : "Synchronous (PLL)");
  utility_add_summary_item(info, "Audio Sample Rate", "%d Hz",
                           AUDIO_SAMPLE_RATE);
  utility_add_summary_item(info, "Filter Cutoff", "%.15g Hz",
