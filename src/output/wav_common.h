...
/**
 * @brief Validates that the selected sample format is compatible with WAV
 * output.
 */
 ...
 * @param sf_format_flag The specific libsndfile format flag (e.g.,
 * SF_FORMAT_WAV or SF_FORMAT_RF64).
 * @return true on success, false on failure.
 */
bool output_wav_common_initialize(ModuleContext *context, int sf_format_flag);

/**
 * @brief Writes a single chunk of raw data to the open file handle. (Used for
 * passthrough mode).
 */
...
/**
 * @brief Finalizes the WAV/RF64 file by closing the handle and updating final
 * size.
 */
