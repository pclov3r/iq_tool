...
typedef struct {
  ...
  int (*pipe_samples_cf32)(nrsc5_t *nrsc5, const float *samples,
                           unsigned int length);
...
static struct {
  ...
  int program_id;
  int active_mode;
} s_nrsc5_config = {.band_str = NULL,
                    .aas_dir_arg = NULL,
                    .program_id = -1,
                    .active_mode = NRSC5_MODE_FM};

static void nrsc5_event_callback(const nrsc5_event_t *event_payload,
                                 void *opaque);
...
static void update_ber_stats(nrsc5_context *nrsc5_decoder, float cber) {
  ...
  if (cber < nrsc5_decoder->ber_min)
    nrsc5_decoder->ber_min = cber;
  if (cber > nrsc5_decoder->ber_max)
    nrsc5_decoder->ber_max = cber;

  log_info("NRSC5: BER: %f, avg: %f, min: %f, max: %f", cber,
           nrsc5_decoder->ber_sum / nrsc5_decoder->ber_count,
           nrsc5_decoder->ber_min, nrsc5_decoder->ber_max);
}

static void sanitize_aas_filename(const char *raw, char *out,
                                  size_t max_length) {
  size_t j = 0;
  if (!raw || max_length == 0) {
    if (max_length > 0)
...
static void dump_aas_file(nrsc5_context *nrsc5_decoder,
                          const nrsc5_event_t *event_payload) {
  ...
  if (fp) {
    ...
    log_warn("NRSC5: Failed to save AAS file '%s': %s", safe_name,
             strerror(errno));
  }
}
...
static void nrsc5_event_callback(const nrsc5_event_t *event_payload,
  ...
  case NRSC5_EVENT_MER:
    log_info("NRSC5: MER: %.1f dB (lower), %.1f dB (upper)",
             event_payload->mer.lower, event_payload->mer.upper);
    break;
  ...
  case NRSC5_EVENT_HDC:
    if (event_payload->hdc.program == nrsc5_decoder->active_program) {
      ...
      if (nrsc5_decoder->audio_packets_valid >= 32) {
        float kbps = (float)nrsc5_decoder->audio_bytes * 8.0f *
                     NRSC5_SAMPLE_RATE_AUDIO / NRSC5_AUDIO_FRAME_SAMPLES /
                     nrsc5_decoder->audio_packets_valid / 1000.0f;
      ...
      if (nrsc5_decoder->audio_packets >= 32) {
        if (nrsc5_decoder->audio_errors > 0) {
          log_warn("NRSC5: Audio CRC errors (recent): %d",
                   nrsc5_decoder->audio_errors);
          log_warn("NRSC5: Audio CRC errors (total): %d",
                   nrsc5_decoder->total_audio_errors);
        }
  ...
  case NRSC5_EVENT_AUDIO:
    if (event_payload->audio.program == nrsc5_decoder->active_program) {
      ...
      audio_output_write(nrsc5_decoder->audio_out, event_payload->audio.data,
                         bytes, nrsc5_decoder->process_chain_mode);
    }
    break;
  ...
  case NRSC5_EVENT_ID3:
    if (event_payload->id3.program == nrsc5_decoder->active_program) {
      ...
      if (event_payload->id3.artist)
        log_info("NRSC5: Artist: %s", event_payload->id3.artist);
      if (event_payload->id3.album)
        log_info("NRSC5: Album: %s", event_payload->id3.album);
      if (event_payload->id3.genre)
        log_info("NRSC5: Genre: %s", event_payload->id3.genre);

      if (event_payload->id3.xhdr.param >= 0) {
        log_info("NRSC5: XHDR: %d %08X %d", event_payload->id3.xhdr.param,
                 event_payload->id3.xhdr.mime, event_payload->id3.xhdr.lot);
      }
    }
    break;

  case NRSC5_EVENT_SIG:
    for (nrsc5_sig_service_t *svc = event_payload->sig.services; svc != NULL;
         svc = svc->next) {
      log_info("NRSC5: SIG Service: type=%s number=%d name=%s",
               svc->type == NRSC5_SIG_SERVICE_AUDIO ? "audio" : "data",
               svc->number, SAFE_STR(svc->name));

      for (nrsc5_sig_component_t *comp = svc->components; comp != NULL;
           comp = comp->next) {
        if (comp->type == NRSC5_SIG_SERVICE_AUDIO) {
          log_info(
              "NRSC5:   Audio component: id=%d port=%04X type=%d mime=%08X",
              comp->id, comp->audio.port, comp->audio.type, comp->audio.mime);
        } else {
          log_info("NRSC5:   Data component: id=%d port=%04X "
                   "service_data_type=%d type=%d mime=%08X",
  ...
  case NRSC5_EVENT_STATION_NAME:
    log_info("NRSC5: Station name: %s",
             SAFE_STR(event_payload->station_name.name));
    break;

  case NRSC5_EVENT_STATION_SLOGAN:
    log_info("NRSC5: Slogan: %s",
             SAFE_STR(event_payload->station_slogan.slogan));
    break;

  case NRSC5_EVENT_STATION_MESSAGE:
    log_info("NRSC5: Message: %s",
             SAFE_STR(event_payload->station_message.message));
    break;

  case NRSC5_EVENT_STATION_ID:
    log_info("NRSC5: Country: %s, FCC facility ID: %d",
             SAFE_STR(event_payload->station_id.country_code),
             event_payload->station_id.fcc_facility_id);
    break;
  ...
  case NRSC5_EVENT_AUDIO_SERVICE_DESCRIPTOR:
    ...
    log_info("NRSC5: Audio program %d: %s, type: %s, sound experience %d",
             ...
             event_payload->asd.access == NRSC5_ACCESS_PUBLIC ? "public"
                                                              : "restricted",
  ...
  case NRSC5_EVENT_DATA_SERVICE_DESCRIPTOR:
    ...
    log_info("NRSC5: Data service: %s, type: %s, MIME type %03x",
             event_payload->dsd.access == NRSC5_ACCESS_PUBLIC ? "public"
                                                              : "restricted",
  ...
  case NRSC5_EVENT_AUDIO_SERVICE:
    ...
    log_info("NRSC5: Audio service %d: %s, type: %s, codec: %d, blend: %d, "
             "gain: %d dB, delay: %d, latency: %d",
             event_payload->audio_service.program,
             event_payload->audio_service.access == NRSC5_ACCESS_PUBLIC
                 ? "public"
                 : "restricted",
             SAFE_STR(name_ptr), event_payload->audio_service.codec_mode,
  ...
  case NRSC5_EVENT_HERE_IMAGE:
    if (event_payload->here_image.time_utc) {
      strftime(time_str, sizeof(time_str), "%Y-%m-%dT%H:%M:%SZ",
               event_payload->here_image.time_utc);
    } else {
      snprintf(time_str, sizeof(time_str), "UNKNOWN_TIME");
    }
    log_info("NRSC5: HERE Image: type=%s, seq=%d, n1=%d, n2=%d, time=%s, "
             "name=%s, size=%d",
             event_payload->here_image.image_type == NRSC5_HERE_IMAGE_TRAFFIC
                 ? "TRAFFIC"
                 : "WEATHER",
             event_payload->here_image.seq, event_payload->here_image.n1,
             event_payload->here_image.n2, time_str,
             SAFE_STR(event_payload->here_image.name),
             event_payload->here_image.size);
    if (s_nrsc5_config.aas_dir_arg)
      dump_aas_file(nrsc5_decoder, event_payload);
    break;

  case NRSC5_EVENT_LOT:
    if (event_payload->lot.expiry_utc) {
      strftime(time_str, sizeof(time_str), "%Y-%m-%dT%H:%M:%SZ",
               event_payload->lot.expiry_utc);
    } else {
      snprintf(time_str, sizeof(time_str), "UNKNOWN_TIME");
    }
    log_info(
        "NRSC5: LOT file: port=%04X lot=%d name=%s size=%d mime=%08X expiry=%s",
        event_payload->lot.component->data.port, event_payload->lot.lot,
        SAFE_STR(event_payload->lot.name), event_payload->lot.size,
        event_payload->lot.mime, time_str);
    if (s_nrsc5_config.aas_dir_arg)
      dump_aas_file(nrsc5_decoder, event_payload);
    break;

  case NRSC5_EVENT_EXCITER_INFO:
    log_info("NRSC5: Exciter: manufacturer=%s, core=%d.%d.%d.%d, status=%d, "
             "importer=%s",
             ...
             event_payload->exciter_info.importer_connected ? "connected"
                                                            : "disconnected");
    break;
  ...
  case NRSC5_EVENT_LOCAL_TIME:
    log_info("NRSC5: Local Time: offset=%d min, regional DST=%d, local DST=%d, "
             "DST schedule=%d",
...
static double calculate_buffer_power(const float complex *samples,
                                     unsigned int frames) {
...
static bool output_nrsc5_validate_options(AppContext *app) {
  ...
  if (!s_nrsc5_config.band_str) {
    ...
    if (strcasecmp(s_nrsc5_config.band_str, "fm") == 0) {
      ...
      log_error("NRSC5: Invalid band '%s'. Valid bands: fm, am.",
                s_nrsc5_config.band_str);
      return false;
    }
  }

  double required_rate =
      (s_nrsc5_config.active_mode == NRSC5_MODE_FM) ? 744187.5 : 46511.71875;

  if (config->baseband_sample_rate.provided) {
    if (config->baseband_sample_rate.rate_hz != required_rate) {
      log_error("NRSC5: Invalid rate %.15g Hz. Selected band requires exactly "
                "%.15g Hz.",
  ...
  if (s_nrsc5_config.program_id < 0 || s_nrsc5_config.program_id > 7) {
    log_error("NRSC5: Invalid program ID %d. Must be 0-7.",
              s_nrsc5_config.program_id);
    return false;
  }
...
static bool output_nrsc5_initialize(ModuleContext *context) {
  ...
  nrsc5_context *nrsc5_decoder = (nrsc5_context *)mem_arena_alloc(
      &app->process_chain.setup_arena, sizeof(nrsc5_context), true);
  if (!nrsc5_decoder)
    return false;
  ...
  log_info("NRSC5: Audio device initialized (%d Hz, %d Channels).",
           NRSC5_AUDIO_SAMPLE_RATE, NRSC5_AUDIO_CHANNELS);

  nrsc5_decoder->process_chain_mode = app->process_chain_mode;
  ...
  nrsc5_decoder->stat_rate_threshold =
      (size_t)(nrsc5_decoder->input_samplerate * CONSOLE_UPDATE_INTERVAL_SEC);
  ...
  if (nrsc5_set_mode(nrsc5_decoder->nrsc5_instance,
                     s_nrsc5_config.active_mode) != 0) {
...
static size_t output_nrsc5_write_chunk(ModuleContext *context,
                                       const void *buffer, size_t input_bytes) {
  AppContext *app = context->app;
  nrsc5_context *nrsc5_decoder =
      (nrsc5_context *)app->module.output_private_data;
  if (input_bytes == 0)
    return 0;
  ...
  if (nrsc5_decoder->stat_rate_threshold > 0 &&
    ...
    double avg_power =
        nrsc5_decoder->accum_mag_sq_sum / (double)nrsc5_decoder->stat_counter;
...
static void output_nrsc5_get_summary_info(const ModuleContext *context,
  ...
  utility_add_summary_item(
      info, "Band", "%s",
      (s_nrsc5_config.active_mode == NRSC5_MODE_FM) ? "FM" : "AM");
  utility_add_summary_item(info, "Program", "%d (HD%d)",
                           s_nrsc5_config.program_id,
