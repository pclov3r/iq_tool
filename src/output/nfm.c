...
static struct {
  ...
  bool disable_discriminator_filter; // 1 = Disable De-emphasis & LPF (Raw/Data
                                     // Mode)
} s_nfm_config = {
    .gain = 1.0f,
    .squelch_snr = 10.0f, // default: opens at 10.0 dB SNR (closes at 8.0 dB due
                          // to hysteresis)
...
static bool output_nfm_validate_options(AppContext *app) {
  ...
  if (rate < NFM_MIN_INPUT_RATE || rate > NFM_MAX_INPUT_RATE) {
    ...
    log_error("Valid range is %.15g Hz to %.15g Hz.", NFM_MIN_INPUT_RATE,
              NFM_MAX_INPUT_RATE);
    return false;
  }
...
static bool output_nfm_initialize(ModuleContext *context) {
  ...
  NfmContext *p = (NfmContext *)mem_arena_alloc(&res->process_chain.setup_arena,
  ...
  p->deemph_filter =
      iirfilt_rrrf_create_lowpass(1, NFM_DEEMPH_FREQ / p->input_samplerate);

  // C. Audio Cleanup Filter (4kHz)
  p->audio_lpf =
      iirfilt_rrrf_create_lowpass(4, NFM_AUDIO_CUTOFF / p->input_samplerate);

  // D. Output Resampler
  p->resampler = msresamp_rrrf_create(
      p->output_ratio, context->config->dsp.filter.args.attenuation);

  // Squelch state
  p->squelch_open = false;
  p->squelch_hang_counter = 0;
  p->squelch_hang_max =
      (int)(p->input_samplerate * (SQ_HANG_TIME_MS / 1000.0f));

  log_info("NFM: Baseband %.15g Hz | Mode: %s | Squelch: %.15g dB | "
           "Discriminator Filter: %s",
           p->input_samplerate,
  ...
  p->mono_buffer = mem_arena_alloc(&res->process_chain.setup_arena,
                                   in_samples * sizeof(float), false);
  p->resamp_buffer = mem_arena_alloc(&res->process_chain.setup_arena,
                                     out_samples * sizeof(float), false);
  p->pcm_out = mem_arena_alloc(&res->process_chain.setup_arena,
...
static size_t output_nfm_write_chunk(ModuleContext *context, const void *buffer,
  ...
  if (_first_run) {
    stat_rate_threshold =
        (size_t)(p->input_samplerate * CONSOLE_UPDATE_INTERVAL_SEC);
    _first_run = false;
  }
  if (input_bytes == 0)
    return 0;
  ...
  if (block_variance > 1e-12f) {
    block_snr_db =
        10.0f * log10f((block_mean_mag * block_mean_mag) / block_variance);
  } else {
  ...
  if (!s_nfm_config.squelch_disabled) {
    // Open squelch immediately if block SNR rises above threshold (e.g., 10.0
    // dB)
    ...
    if (p->squelch_open) {
      ...
      float snr_db =
          ...
          log10f((float)((mean_mag * mean_mag) /
                         fmax(1e-12, avg_power - (mean_mag * mean_mag))));
  ...
  for (unsigned int i = 0; i < n; i++) {
    ...
    if (!p->squelch_open)
      sample = 0.0f; // Mute silent buffers cleanly
    p->mono_buffer[i] = sample;
  }

  // 6. Resample to 48kHz audio and output
  unsigned int num_resampled;
  msresamp_rrrf_execute(p->resampler, p->mono_buffer, n, p->resamp_buffer,
                        &num_resampled);
  interleave_f32_to_s16(p->resamp_buffer, p->resamp_buffer, p->pcm_out,
                        num_resampled);
  audio_output_write(p->audio_out, p->pcm_out,
...
static void output_nfm_cleanup(ModuleContext *context) {
  ...
  if (p->fm_demod)
    freqdem_destroy(p->fm_demod);
  if (p->deemph_filter)
    iirfilt_rrrf_destroy(p->deemph_filter);
  if (p->audio_lpf)
    iirfilt_rrrf_destroy(p->audio_lpf);
  if (p->resampler)
    msresamp_rrrf_destroy(p->resampler);
}
...
static void output_nfm_get_summary_info(const ModuleContext *context,
  ...
  const char *type = s_nfm_config.disable_discriminator_filter
                         ? "Discriminator Filter Disabled"
                         : "Discriminator Filter Enabled";
  ...
  utility_add_summary_item(info, "Squelch", "%.15g dB",
                           s_nfm_config.squelch_snr);
}
