...
static bool output_nfm_initialize(ModuleContext *context) {
  ...
  float kf = selected_dev / nfm_decoder->input_samplerate;
  ...
  nfm_decoder->resampler = msresamp_rrrf_create(
      nfm_decoder->output_ratio, context->config->dsp.filter.args.attenuation);

  // Squelch state
  nfm_decoder->squelch_open = false;
  nfm_decoder->squelch_hang_counter = 0;
  nfm_decoder->squelch_hang_max =
      (int)(nfm_decoder->input_samplerate * (SQ_HANG_TIME_MS / 1000.0f));
...
static void output_nfm_flush(ModuleContext *context) {
  ...
  if (is_shutdown_requested()) {
    ...
    audio_output_drain(nfm_decoder->audio_out);
  }
}
...
// Statics moved to Context struct
  ...
  if (!s_nfm_config.squelch_disabled) {
    ...
    if (block_snr_db > s_nfm_config.squelch_snr) {
      nfm_decoder->squelch_open = true;
      nfm_decoder->squelch_hang_counter = nfm_decoder->squelch_hang_max;
    }
  ...
  if (nfm_decoder->stat_counter >= nfm_decoder->stat_rate_threshold) {
    ...
    nfm_decoder->stat_counter = 0;
    nfm_decoder->accum_mag_sum = 0.0;
    nfm_decoder->accum_mag_sq_sum = 0.0;
  }
  ...
  for (unsigned int i = 0; i < n; i++) {
    ...
    if (!s_nfm_config.disable_discriminator_filter) {
      ...
      iirfilt_rrrf_execute(nfm_decoder->audio_lpf, sample, &sample);
    }
    sample *= s_nfm_config.gain;
    if (!nfm_decoder->squelch_open)
      sample = 0.0f; // Mute silent buffers cleanly
    nfm_decoder->mono_buffer[i] = sample;
  }
...
static void output_nfm_cleanup(ModuleContext *context) {
  ...
  if (nfm_decoder->fm_demod)
    freqdem_destroy(nfm_decoder->fm_demod);
  if (nfm_decoder->deemph_filter)
    iirfilt_rrrf_destroy(nfm_decoder->deemph_filter);
  if (nfm_decoder->audio_lpf)
    iirfilt_rrrf_destroy(nfm_decoder->audio_lpf);
  if (nfm_decoder->resampler)
    msresamp_rrrf_destroy(nfm_decoder->resampler);
}
