...
// --- Miniaudio Callback ---
static void miniaudio_data_callback(ma_device *pDevice, void *pOutput,
  ...
  if (available < bytes_needed) {
    ...
    if (available > 0)
      ring_buffer_read(context->audio_ring_buffer, pOutput, available);
...
AudioOutputContext *audio_output_create(AppContext *app, int sample_rate,
                                        int channels,
                                        size_t buffer_size_bytes) {
  AppConfig *config = (AppConfig *)app->config;
  AudioOutputContext *context = (AudioOutputContext *)mem_arena_alloc(
      &app->process_chain.setup_arena, sizeof(AudioOutputContext), true);
  if (!context)
    return NULL;
...
size_t audio_output_write(AudioOutputContext *context, const void *pcm_data,
                          size_t bytes, ProcessChainMode mode) {
  if (!context || bytes == 0)
    return 0;

  if (context->wav_writer) {
    sf_write_short(context->wav_writer, (const short *)pcm_data,
  ...
  size_t safe_capacity =
      (context->buffer_size > 0) ? context->buffer_size - 1 : 0;
  ...
  while (bytes_left > 0) {
    ...
    if (mode == PROCESS_CHAIN_MODE_SYNCHRONOUS_PULL) {
    ...
    size_t written =
        ring_buffer_write(context->audio_ring_buffer, ptr, chunk_size);
    if (written > 0) {
      ptr += written;
      bytes_left -= written;
    } else {
      // Write failed (buffer likely shutting down), break to prevent infinite
      // loop
      break;
    }
  }
...
void audio_output_drain(AudioOutputContext *context) {
  if (!context || !context->audio_ring_buffer)
    return;
