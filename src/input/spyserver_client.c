...
#define SPYSERVER_PROTOCOL_VERSION                                             \
  (((uint32_t)2 << 24) | ((uint32_t)0 << 16) | (1700))
...
static SampleFormat
get_internal_format_from_spyserver_enum(int spyserver_format) {
  for (size_t i = 0; i < sizeof(format_map) / sizeof(format_map[0]); i++) {
...
static bool send_setting(SpyServerClientContext *client, uint32_t setting,
                         uint32_t value) {
  unsigned char command_buffer[sizeof(SpyServerCommandHeader) +
                               sizeof(SpyServerSettingTarget)];
  ...
  SpyServerSettingTarget *payload =
      (SpyServerSettingTarget *)(command_buffer +
                                 sizeof(SpyServerCommandHeader));
  payload->Setting = setting;
  payload->Value = value;

  return networking_send_all(client->net_context, command_buffer,
...
static bool input_spyserver_client_validate_options(AppContext *app) {
  ...
  if (s_spyserver_client_config.sample_format_str != NULL) {
    if (strcasecmp(s_spyserver_client_config.sample_format_str, "cu8") != 0 &&
      ...
      log_error("Invalid value for --spyserver-client-sample-format: '%s'. "
                "Must be one of {cu8|cs16|cs24|cf32}.",
...
static bool discard_network_bytes(NetworkingContext *net_context,
                                  size_t bytes_to_discard) {
  char discard_buffer[1024];
  while (bytes_to_discard > 0) {
    size_t to_read = (bytes_to_discard > sizeof(discard_buffer))
...
static bool input_spyserver_client_initialize(ModuleContext *context) {
  ...
  SpyServerClientContext *client = (SpyServerClientContext *)mem_arena_alloc(
      &app->process_chain.setup_arena, sizeof(SpyServerClientContext), true);
  if (!client)
    return false;
  ...
  client->rx_buffer = (unsigned char *)mem_arena_alloc(
      &app->process_chain.setup_arena, client->rx_buffer_size, false);
  if (!client->rx_buffer)
    return false;

  // This module now takes responsibility for initializing its dependency.
  if (!networking_init()) {
    log_error("SpyServer client failed because the networking module could not "
              "be initialized.");
    return false;
  }

  client->net_context = networking_connect(s_spyserver_client_config.hostname,
  ...
  unsigned char *payload_buffer = (unsigned char *)mem_arena_alloc(
      &app->process_chain.setup_arena, payload_size, false);
  if (!payload_buffer) {
    goto error_cleanup;
  }
  ...
  bool send_ok =
      networking_send_all(client->net_context, &hello_header,
                          sizeof(hello_header)) &&
  ...
  if (!networking_recv_all(client->net_context, &response_header,
                           sizeof(response_header))) {
    goto error_cleanup;
  }

  if (response_header.MessageType != SPYSERVER_MSG_TYPE_DEVICE_INFO) {
    log_error("Did not receive DeviceInfo after handshake. Server may have "
              "rejected the connection (MessageType=%u).",
              response_header.MessageType);
    goto error_cleanup;
  }
  if (response_header.BodySize < sizeof(SpyServerDeviceInfo)) {
    log_error("Received DeviceInfo with unexpected size (%u vs %zu).",
              response_header.BodySize, sizeof(SpyServerDeviceInfo));
    goto error_cleanup;
  }
  if (!networking_recv_all(client->net_context, &client->device_info,
                           sizeof(SpyServerDeviceInfo))) {
    goto error_cleanup;
  }
  size_t dev_extra_bytes =
      response_header.BodySize - sizeof(SpyServerDeviceInfo);
  ...
  while (!got_sync && ignored < max_ignored) {
    ...
    if (response_header.MessageType == SPYSERVER_MSG_TYPE_CLIENT_SYNC) {
      if (response_header.BodySize < sizeof(SpyServerClientSync)) {
        log_error("Received ClientSync with unexpected size (%u vs %zu). "
                  "Protocol mismatch.",
                  response_header.BodySize, sizeof(SpyServerClientSync));
        goto error_cleanup;
      }
      if (!networking_recv_all(client->net_context, &sync_info,
                               sizeof(SpyServerClientSync))) {
        goto error_cleanup;
      }
      size_t sync_extra =
          response_header.BodySize - sizeof(SpyServerClientSync);
  ...
  log_info("Client has control of the remote device. Negotiating stream "
           "parameters...");

  // Determine the format our client wants to request based on user args or
  // defaults.
  const SampleFormatInfo *req_fmt_info =
      get_format_info_by_name(s_spyserver_client_config.sample_format_str);
  SampleFormat requested_format =
      req_fmt_info ? req_fmt_info->format_enum : FORMAT_UNKNOWN;
  log_info("Client requesting sample format: %s",
           req_fmt_info ? req_fmt_info->description_str : "Unknown");
  ...
  if (forced_format_enum != 0) {
    SampleFormat server_forced_format =
        get_internal_format_from_spyserver_enum(forced_format_enum);

    // Only warn and switch if the server's required format is valid and
    // DIFFERENT from what we were going to request.
    if (server_forced_format != FORMAT_UNKNOWN &&
        server_forced_format != requested_format) {
      log_warn(
          "Server requires the %s sample format. Switching...",
          get_format_info_by_enum(server_forced_format)
              ? get_format_info_by_enum(server_forced_format)->description_str
              : "Unknown");
  ...
  double user_rate = config->sdr_general.sample_rate_hz > 0
  ...
  if (min_diff < 1.0) {
    ...
    log_info("Requested sample rate %.15g Hz. Using closest available rate: "
             "%.15g Hz.",
  ...
  if (!send_setting(client, SPYSERVER_SETTING_IQ_FREQUENCY,
                    (uint32_t)config->sdr_general.rf_freq_hz))
    goto error_cleanup;
  if (!send_setting(client, SPYSERVER_SETTING_IQ_DECIMATION, dec_index_to_send))
    goto error_cleanup;
  if (!send_setting(client, SPYSERVER_SETTING_IQ_FORMAT, format_to_request_int))
    goto error_cleanup;
  ...
  if (s_spyserver_client_config.gain_provided) {
    ...
    if (client->device_info.MaximumGainIndex > 0) {
      ...
      if (requested_gain > client->device_info.MaximumGainIndex) {
        ...
        log_error("Valid gain range for this server is 0 to %u.",
                  client->device_info.MaximumGainIndex);
      ...
      // Store the successfully applied gain for the digital gain calculation
      // below
      effective_gain_index = requested_gain;

    } else {
      // If the server simply doesn't support gain (e.g. Airspy HF+ or fixed
      // config), warn but proceed.
      log_warn(
          "Manual gain requested, but not supported by this server. Ignoring.");
  ...
  if (device_type == SPYSERVER_DEV_AIRSPY_ONE) {
    // Airspy One specific formula requires the Gain Index we just
    // determined/validated
    digital_gain_float =
        (float)(client->device_info.MaximumGainIndex - effective_gain_index) +
        ((float)dec_index_to_send * 3.01f);
  } else {
    // Airspy HF+, RTL-SDR, and others do NOT use the Gain Index in this
    // calculation
    digital_gain_float = (float)dec_index_to_send * 3.01f;
  }

  if (!send_setting(client, SPYSERVER_SETTING_IQ_DIGITAL_GAIN,
                    (uint32_t)digital_gain_float))
    goto error_cleanup;

  if (!send_setting(client, SPYSERVER_SETTING_STREAMING_MODE,
                    SPYSERVER_STREAM_MODE_IQ_ONLY))
    goto error_cleanup;

  // Dynamic Ring Buffer Sizing
  double bytes_per_sec =
      (double)actual_rate * (double)app->module.input_bytes_per_iq_sample;

  // Calculate total capacity based on the pre-buffer target and the headroom
  // factor. e.g. 2.5s * 4.0 = 10.0s of total capacity.
  size_t desired_buffer_size =
      (size_t)(bytes_per_sec * SPYSERVER_PREBUFFER_TARGET_SECONDS *
               SPYSERVER_BUFFER_HEADROOM_FACTOR);

  // Sanity Clamp: Minimum 1MB
  if (desired_buffer_size < SPYSERVER_RING_BUFFER_MIN_BYTES)
    desired_buffer_size = SPYSERVER_RING_BUFFER_MIN_BYTES;

  // Hard Ceiling: Never exceed the absolute max defined in constants.h
  if (desired_buffer_size > SPYSERVER_MAX_BUFFER_BYTES)
    desired_buffer_size = SPYSERVER_MAX_BUFFER_BYTES;

  log_info("SpyServer Ring Buffer: Allocating %zu bytes (%.2f sec capacity) "
           "for %.2f sec pre-buffer target.",
           desired_buffer_size, (double)desired_buffer_size / bytes_per_sec,
           SPYSERVER_PREBUFFER_TARGET_SECONDS);

  client->stream_buffer =
      ring_buffer_create(desired_buffer_size, &app->process_chain.setup_arena);
...
static void *input_spyserver_client_producer_thread(void *arg) {
  ...
  while (!is_shutdown_requested()) {
    ...
    bool is_iq_data = (msg_type >= SPYSERVER_MSG_TYPE_UINT8_IQ &&
                       msg_type <= SPYSERVER_MSG_TYPE_FLOAT_IQ);
    ...
    if (is_iq_data && body_size > 0) {
      ...
      while (bytes_remaining > 0 && !is_shutdown_requested()) {
        // Determine how much to read in this chunk (limited by scratch buffer
        // size)
        size_t chunk_size = (bytes_remaining > client->rx_buffer_size)
        ...
        if (aligned_read == 0) {
          // We have fewer bytes left than a single sample requires (e.g. 1 byte
          // left for 2-byte format). We MUST consume these from the socket or
          // the next header read will be desynchronized. We use bytes_remaining
        ...
        PacketWriteResult write_res = packet_serializer_write_packet(
            client->stream_buffer, samples_in_chunk, client->rx_buffer,
            client->active_format, (double)app->module.input_info.sample_rate);

        if (write_res == PACKET_WRITE_INACTIVE) {
          goto end_loop;
        } else if (write_res == PACKET_WRITE_DROPPED) {
          ...
          if (current_time - last_drop_log_time >=
            ...
            log_warn("SpyServer: Ring buffer full! Dropped %zu samples.",
                     accumulated_drops);
            accumulated_drops = 0;
      ...
      // --- METADATA / INFO / SYNC ---
      // We discard these packets to prevent them from entering the processing
      // process_chain.
      if (body_size > 0) {
...
static void
input_spyserver_client_stop_sample_queue_push(ModuleContext *context) {
  AppContext *app = context->app;
...
input_spyserver_client_get_summary_info(const ModuleContext *context,
  ...
  snprintf(server_addr, sizeof(server_addr), "%s:%d",
           s_spyserver_client_config.hostname, s_spyserver_client_config.port);
  ...
  if (client && client->device_info_ok) {
    ...
    case SPYSERVER_DEV_AIRSPY_ONE:
      dev_type_str = "Airspy One";
      break;
    case SPYSERVER_DEV_AIRSPY_HF:
      dev_type_str = "Airspy HF+";
      break;
    case SPYSERVER_DEV_RTLSDR:
      dev_type_str = "RTL-SDR";
      break;
    }
    char dev_info_str[128];
    snprintf(dev_info_str, sizeof(dev_info_str), "%s (S/N: %08X)", dev_type_str,
             client->device_info.DeviceSerial);
    utility_add_summary_item(info, "Remote Device", dev_info_str);
    utility_add_summary_item(
        info, "Input Format",
        get_format_info_by_enum(app->module.input_format)
    ...
    if (s_spyserver_client_config.gain_provided) {
      utility_add_summary_item(info, "Gain", "%d (Manual)",
                               s_spyserver_client_config.gain);
    } else {
