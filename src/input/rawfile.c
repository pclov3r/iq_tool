...
} s_rawfile_config = {.sample_rate_hz = 0.0,
...
static bool input_rawfile_validate_options(AppContext *app) {
  ...
  if (s_rawfile_config.raw_file_sample_rate_hz_arg > 0.0f) {
    s_rawfile_config.sample_rate_hz =
        (double)s_rawfile_config.raw_file_sample_rate_hz_arg;
  ...
  if (!s_rawfile_config.sample_rate_provided) {
    log_error("Missing required option --rawfile-input-rate <hz> for raw file "
              "input.");
    return false;
  }
  if (!format_provided) {
    log_error("Missing required option --rawfile-input-sample-format <format> "
              "for raw file input.");
    return false;
  }
  ...
  RawfileInputContext *private_data = (RawfileInputContext *)mem_arena_alloc(
      &app->process_chain.setup_arena, sizeof(RawfileInputContext), true);
  if (!private_data)
    return false;
  app->module.input_private_data = private_data;

  SampleFormat format_enum =
      get_format_info_by_name(s_rawfile_config.format_str)
          ? get_format_info_by_name(s_rawfile_config.format_str)->format_enum
          : FORMAT_UNKNOWN;
  if (format_enum == FORMAT_UNKNOWN) {
    log_error("Invalid RAW input format '%s'. See --help for valid formats.",
              s_rawfile_config.format_str);
    return false;
  }
...
static bool input_rawfile_initialize(ModuleContext *context) {
  ...
  RawfileInputContext *private_data =
      (RawfileInputContext *)app->module.input_private_data;
  ...
  if (app->module.input_bytes_per_iq_sample == 0) {
    log_fatal(
        "Internal error: could not determine sample size for format '%s'.",
        s_rawfile_config.format_str);
    return false;
  }
...
static size_t input_rawfile_read_chunk(ModuleContext *context, void *buffer,
                                       size_t bytes_to_read) {
  AppContext *app = context->app;
  RawfileInputContext *p =
      (RawfileInputContext *)app->module.input_private_data;
  if (!p || !p->infile || bytes_to_read == 0)
    return 0;
  ...
  while (bytes_left > 0) {
    ...
    sf_count_t read_this_pass =
        sf_read_raw(p->infile, (char *)buffer + bytes_read_total, bytes_left);
...
static void *input_rawfile_push_samples_to_queue(ModuleContext *context,
...
static bool input_rawfile_pre_stream_iq_correction(ModuleContext *context) {
  ...
  RawfileInputContext *private_data =
      (RawfileInputContext *)context->app->module.input_private_data;
  ...
  bool result = iq_correction_run_initial_calibration(
      context, raw_buffer, raw_buffer_size, raw_iq_cal_read_cb,
      private_data->infile);
