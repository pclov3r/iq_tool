...
typedef struct {
  ...
  int (*set_sample_rate)(struct bladerf *, bladerf_channel, bladerf_sample_rate,
                         bladerf_sample_rate *);
  int (*set_rational_sample_rate)(struct bladerf *, bladerf_channel,
                                  struct bladerf_rational_rate *,
                                  struct bladerf_rational_rate *);
  int (*enable_feature)(struct bladerf *, bladerf_feature, bool);
  int (*set_bandwidth)(struct bladerf *, bladerf_channel, bladerf_bandwidth,
                       bladerf_bandwidth *);
  ...
  int (*sync_config)(struct bladerf *, bladerf_channel_layout, bladerf_format,
                     unsigned int, unsigned int, unsigned int, unsigned int);
  int (*enable_module)(struct bladerf *, bladerf_module, bool);
  int (*sync_rx)(struct bladerf *, void *, unsigned int,
                 struct bladerf_metadata *, unsigned int);
  int (*init_stream)(struct bladerf_stream **, struct bladerf *,
                     bladerf_stream_cb, void ***, size_t, bladerf_format,
                     size_t, size_t, void *);
...
static bool bladerf_configure_standard_rate_and_rf(ModuleContext *context,
                                                   bladerf_channel rx_channel);
static bool
bladerf_configure_high_speed_rate_and_rf(ModuleContext *context,
                                         bladerf_channel rx_channel);
...
static bool input_bladerf_validate_options(AppContext *app) {
  ...
  if (s_bladerf_config.bladerf_bandwidth_hz_arg != 0.0f) {
    ...
    s_bladerf_config.bandwidth_hz =
        (uint32_t)s_bladerf_config.bladerf_bandwidth_hz_arg;
  ...
  if (config->sdr_general.sample_rate_hz > 61440000.0) {
    if (s_bladerf_config.bit_depth_provided &&
        s_bladerf_config.bit_depth_arg == 12) {
      log_error("Invalid configuration: The BladeRF does not support 12-bit "
                "mode for sample rates above 61440000 Hz.");
      return false;
    }
    if (!s_bladerf_config.bit_depth_provided) {
      log_warn("Sample rate of %.15g Hz exceeds the 61440000 Hz limit for "
               "12-bit mode. Automatically switching to 8-bit mode.",
               config->sdr_general.sample_rate_hz);
    }
    s_bladerf_config.active_bit_depth = 8;
  } else {
    if (s_bladerf_config.bit_depth_provided) {
      if (s_bladerf_config.bit_depth_arg == 8 ||
          s_bladerf_config.bit_depth_arg == 12) {
  ...
  if (s_bladerf_config.active_bit_depth == 8 &&
      s_bladerf_config.bandwidth_provided) {
    log_error("Option --bladerf-bandwidth cannot be used with 8-bit high-speed "
              "mode.");
    log_error("In this mode, the analog bandwidth is configured automatically "
              "by the library.");
    return false;
  }
...
static bool input_bladerf_initialize(ModuleContext *context) {
  ...
  BladerfContext *private_data = (BladerfContext *)mem_arena_alloc(
      &app->process_chain.setup_arena, sizeof(BladerfContext), true);
  if (!private_data)
    goto cleanup; // mem_arena_alloc logs the error
  ...
  if (s_bladerf_config.device_index > 0) {
    snprintf(device_identifier, sizeof(device_identifier), "bladerf%d",
             s_bladerf_config.device_index);
  } else {
  ...
  status = bladerf_open(&private_data->dev,
                        device_identifier[0] ? device_identifier : NULL);
  if (is_shutdown_requested())
    goto cleanup;
  ...
  if (s_bladerf_config.fpga_file_path) {
    ...
    status =
        bladerf_load_fpga(private_data->dev, s_bladerf_config.fpga_file_path);
    if (is_shutdown_requested())
      goto cleanup;
    if (status != 0) {
      log_error("Failed to load specified BladeRF FPGA: %s",
    ...
    if (is_shutdown_requested())
      goto cleanup;
    if (status < 0) {
      log_error("Failed to query BladeRF FPGA state: %s",
                bladerf_strerror(status));
      goto cleanup;
    }
    if (status == 0) {
      log_info("BladeRF FPGA not configured. Attempting to find and load it "
               "automatically...");
  ...
  strncpy(private_data->board_name, board_name_from_api,
          sizeof(private_data->board_name) - 1);
  ...
  if (s_bladerf_config.active_bit_depth == 8 && !is_bladerf2) {
    log_error("Invalid configuration: 8-bit mode is only supported on BladeRF "
              "2.0 devices.");
    goto cleanup;
  }
  ...
  if (status != 0) {
    log_warn("Could not retrieve BladeRF serial number: %s",
             bladerf_strerror(status));
    strncpy(private_data->serial, "????????", sizeof(private_data->serial) - 1);
  } else {
    strncpy(private_data->serial, serial_struct.serial,
            sizeof(private_data->serial) - 1);
  }
  ...
  if (is_bladerf2) {
    ...
    if (s_bladerf_config.channel != 0) {
      log_warn("Option --bladerf-channel is for BladeRF 2.0 only and is "
               "ignored on this BladeRF 1.0 device.");
    }
  }
  ...
  if (is_high_speed_mode) {
    if (!is_bladerf2) {
      log_error("Invalid configuration: Sample rates above 61440000 Hz are "
                "only supported on BladeRF 2.0 devices.");
      goto cleanup;
    }
    if (!bladerf_configure_high_speed_rate_and_rf(context, rx_channel))
      goto cleanup;
  } else {
    if (!bladerf_configure_standard_rate_and_rf(context, rx_channel))
      goto cleanup;
  }
  ...
  if (s_bladerf_config.gain_provided) {
    status =
        bladerf_set_gain_mode(private_data->dev, rx_channel, BLADERF_GAIN_MGC);
    if (status == 0)
      status = bladerf_set_gain(private_data->dev, rx_channel,
                                s_bladerf_config.gain);
  } else {
    status = bladerf_set_gain_mode(private_data->dev, rx_channel,
  ...
  if (config->sdr_general.bias_t_enable) {
    if (is_bladerf2) {
      ...
      if (is_shutdown_requested())
        goto cleanup;
      if (status != 0) {
        log_error("Failed to enable BladeRF Bias-T: %s",
                  bladerf_strerror(status));
        goto cleanup;
      }
    } else {
      log_warn("Bias-T is not supported on this BladeRF model (%s) and will be "
               "ignored.",
  ...
  app->module.input_format =
      (s_bladerf_config.active_bit_depth == 8) ? CS8 : SC16Q11;
...
bladerf_configure_high_speed_rate_and_rf(ModuleContext *context,
  ...
  status = bladerf_enable_feature(private_data->dev, BLADERF_FEATURE_OVERSAMPLE,
                                  true);
  if (status != 0) {
    log_error("Failed to enable BladeRF oversample feature: %s",
              bladerf_strerror(status));
    return false;
  }

  struct bladerf_rational_rate rate_to_set = {
      .integer = 0,
      .num = (uint64_t)config->sdr_general.sample_rate_hz,
      .den = 1};

  struct bladerf_rational_rate actual_rate_from_device;
  status = bladerf_set_rational_sample_rate(
      private_data->dev, rx_channel, &rate_to_set, &actual_rate_from_device);
  if (status != 0) {
    log_error("Failed to set BladeRF 2.0 rational sample rate: %s",
              bladerf_strerror(status));
    return false;
  }

  if (actual_rate_from_device.den == 0) {
    log_error("BladeRF returned an invalid rational sample rate (denominator "
              "is zero).");
    return false;
  }
  ...
  log_info("BladeRF: Bandwidth is set automatically by the library in "
           "high-speed mode.");
  return true;
}
...
static bool bladerf_configure_standard_rate_and_rf(ModuleContext *context,
  ...
  bladerf_sample_rate requested_rate =
      (bladerf_sample_rate)config->sdr_general.sample_rate_hz;
  bladerf_sample_rate actual_rate;
  status = bladerf_set_sample_rate(private_data->dev, rx_channel,
                                   requested_rate, &actual_rate);
  if (is_shutdown_requested()) {
    return false;
  }
  if (status != 0) {
    log_error("Failed to set BladeRF sample rate: %s",
              bladerf_strerror(status));
    return false;
  }
  log_info(
  ...
  status = bladerf_set_bandwidth(private_data->dev, rx_channel, requested_bw,
                                 &actual_bw);
  if (is_shutdown_requested()) {
    return false;
  }
  ...
  if (requested_bw == 0)
    ...
    log_info(
        "BladeRF: Requested bandwidth %u Hz, actual bandwidth set to %u Hz.",
...
static void *bladerf_rx_stream_callback(struct bladerf *dev,
  ...
  if (!app->module.queue_samples(app->module.process_chain_context, samples,
                                 num_samples, app->module.input_format)) {
...
static void *input_bladerf_push_samples_to_queue(ModuleContext *context,
  ...
  bool is_bladerf2 = (strcmp(private_data->board_name, "bladerf2") == 0);
  rx_channel = is_bladerf2 ? BLADERF_CHANNEL_RX(s_bladerf_config.channel)
  ...
  if (samples_per_buffer > 65536)
    samples_per_buffer = 65536;
  samples_per_buffer = (samples_per_buffer / 2048) * 2048;
  if (samples_per_buffer < 2048)
    samples_per_buffer = 2048;

  log_info("BladeRF: Driver transfer size aligned to %u samples.",
           samples_per_buffer);
  ...
  status = bladerf_init_stream(
      &private_data->rx_stream, private_data->dev, bladerf_rx_stream_callback,
...
static void input_bladerf_get_summary_info(const ModuleContext *context,
  ...
  BladerfContext *private_data =
      (BladerfContext *)app->module.input_private_data;
  utility_add_summary_item(info, "Input Source", "%s",
                           private_data->display_name);

  if (s_bladerf_config.active_bit_depth == 8)
    utility_add_summary_item(info, "Input Format",
                             "8-bit Signed Complex (cs8)");
  else
    utility_add_summary_item(info, "Input Format",
                             "12-bit Signed Complex Q4.11 (sc16q11)");

  if (strcmp(private_data->board_name, "bladerf2") == 0)
    utility_add_summary_item(info, "Channel", "%d (RXA)",
                             s_bladerf_config.channel);
  else
  ...
  if (s_bladerf_config.gain_provided)
    utility_add_summary_item(info, "Gain", "%d dB (Manual)",
                             s_bladerf_config.gain);
  else
...
static bool bladerf_find_and_load_fpga_automatically(struct bladerf *dev) {
  ...
  if (is_shutdown_requested())
    return false;
  if (status != 0) {
    log_error("Could not determine BladeRF FPGA size: %s",
              bladerf_strerror(status));
    return false;
  }
  ...
  case BLADERF_FPGA_40KLE:
    filename_utf8 = "hostedx40.rbf";
    break;
  case BLADERF_FPGA_115KLE:
    filename_utf8 = "hostedx115.rbf";
    break;
  case BLADERF_FPGA_A4:
    filename_utf8 = "hostedxA4.rbf";
    break;
  case BLADERF_FPGA_A5:
    filename_utf8 = "hostedxA5.rbf";
    break;
  case BLADERF_FPGA_A9:
    filename_utf8 = "hostedxA9.rbf";
    break;
  default:
    log_error("Unknown or unsupported BladeRF FPGA size (%d). Cannot determine "
              "FPGA file.",
              fpga_size);
    return false;
  }
  ...
  log_error("Could not automatically find the required FPGA file '%s'.",
            filename_utf8);
  log_error("Please ensure the FPGA files are in the 'fpga/bladerf' "
            "subdirectory next to the executable, or installed system-wide.");
  return false;
}

// --- The InputModuleInterface V-Table ---
static InputModuleInterface s_input_bladerf_api = {
    .initialize = input_bladerf_initialize,
    .push_samples_to_queue = input_bladerf_push_samples_to_queue,
    .stop_sample_queue_push = input_bladerf_stop_sample_queue_push,
    .cleanup = input_bladerf_cleanup,
    .get_summary_info = input_bladerf_get_summary_info,
    .validate_options = input_bladerf_validate_options,
    .validate_generic_options = input_bladerf_validate_generic_options,
