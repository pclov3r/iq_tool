...
static bool input_rtlsdr_validate_options(AppContext *app) {
  ...
  if (s_rtlsdr_config.direct_sampling_mode != 0) {
    if (s_rtlsdr_config.direct_sampling_mode < 1 ||
        s_rtlsdr_config.direct_sampling_mode > 2) {
  ...
  if (config->sdr_general.sample_rate_provided) {
    if (config->sdr_general.sample_rate_hz < 225001 ||
        config->sdr_general.sample_rate_hz > 3200000) {
      log_error("Invalid sample rate for RTL-SDR: %.15g Hz. Must be between "
                "225001 and 3200000.",
...
static void input_rtlsdr_stream_callback(unsigned char *buffer, uint32_t length,
                                         void *cb_context) {
  ...
  // We no longer chop this up. We send the full hardware buffer to the ring
  // buffer.
  uint32_t num_samples = length / 2;

  if (!app->module.queue_samples(app->module.process_chain_context, buffer,
                                 num_samples, CU8)) {
...
static bool input_rtlsdr_initialize(ModuleContext *context) {
  ...
  RtlSdrContext *private_data = (RtlSdrContext *)mem_arena_alloc(
      &app->process_chain.setup_arena, sizeof(RtlSdrContext), true);
  if (!private_data) {
    return false;
  }
  ...
  if (device_index >= device_count) {
    log_error("Device index %u is out of range. Found %u devices.",
              device_index, device_count);
    goto cleanup;
  }
  if (rtlsdr_get_device_usb_strings(device_index, private_data->manufact,
                                    private_data->product,
                                    private_data->serial) < 0) {
    log_fatal("Failed to read USB device strings for device %d.", device_index);
    goto cleanup;
  }

  log_info("Opening device %d: %s %s, S/N: %s", device_index,
  ...
  result = rtlsdr_set_sample_rate(private_data->dev,
                                  (uint32_t)config->sdr_general.sample_rate_hz);
  if (result < 0) {
  ...
  if (s_rtlsdr_config.gain_provided) {
    ...
    int nearest = rtlsdr_find_nearest_gain(private_data->dev, requested,
  ...
  if (s_rtlsdr_config.ppm_provided) {
    ...
    log_info("RTL-SDR: Using manually provided PPM correction: %d",
             resolved_ppm);
  } else {
    int temp_ppm = 0;
    ...
    if (sscanf(private_data->serial, "%*3[PpMm]%d", &temp_ppm) == 1) {
      ...
      log_info("RTL-SDR: Auto-detected PPM correction from serial: %d",
               resolved_ppm);
    }
  }
  ...
  if (resolved_ppm != 0) {
    ...
    if (result < 0) {
      if (ppm_from_serial) {
        log_warn("RTL-SDR: Failed to set automatic PPM correction from serial "
                 "(%d): %s",
                 resolved_ppm, strerror(-result));
      } else {
        log_warn("RTL-SDR: Failed to set manual PPM correction (%d): %s",
                 resolved_ppm, strerror(-result));
      }
    }
  }
  ...
  if (config->sdr_general.bias_t_enable) {
    ...
    if (result != 0) {
      log_warn(
          "Failed to enable Bias-T. The device may not support this feature.");
    }
  }

  if (s_rtlsdr_config.direct_sampling_provided) {
    rtlsdr_set_direct_sampling(private_data->dev,
                               s_rtlsdr_config.direct_sampling_mode);
  }
  ...
  // RingBuffer. This decouples the USB read timing from the output pipe
  // backpressure, preventing sample drops when piping to downstream tools.
...
static void *input_rtlsdr_push_samples_to_queue(ModuleContext *context,
...
static void input_rtlsdr_get_summary_info(const ModuleContext *context,
  ...
  utility_add_summary_item(info, "Bias-T", "%s",
                           config->sdr_general.bias_t_enable ? "Enabled"
                                                             : "Disabled");
  if (s_rtlsdr_config.ppm_provided) {
    utility_add_summary_item(info, "PPM Correction", "%d", s_rtlsdr_config.ppm);
  }
}
