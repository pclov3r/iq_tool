...
static void input_airspy_set_default_config(AppConfig *config) {
...
static const struct argparse_option input_airspy_cli_options[] = {
    OPT_GROUP("Airspy Input (airspy)"),
    OPT_STRING(
        0, "airspy-gain-mode", &s_airspy_config.gain_mode,
        "Gain mode: 'linearity', 'sensitivity', or 'manual'. (Default: AGC)",
        NULL, 0, 0),
    OPT_INTEGER(
        0, "airspy-gain-value", &s_airspy_config.airspy_gain_value_arg,
        "Gain value for linearity/sensitivity modes (0-21). (Default: 10)",
        NULL, 0, 0),
    OPT_INTEGER(0, "airspy-lna-gain", &s_airspy_config.airspy_lna_gain_arg,
                "Manual LNA gain (0-14). Only with manual mode. (Default: 5)",
                NULL, 0, 0),
    OPT_INTEGER(0, "airspy-mixer-gain", &s_airspy_config.airspy_mixer_gain_arg,
                "Manual Mixer gain (0-15). Only with manual mode. (Default: 5)",
                NULL, 0, 0),
    OPT_INTEGER(0, "airspy-vga-gain", &s_airspy_config.airspy_vga_gain_arg,
                "Manual VGA gain (0-15). Only with manual mode. (Default: 5)",
                NULL, 0, 0),
    OPT_STRING(0, "airspy-sample-format", &s_airspy_config.sample_format,
               "Sample format: 'cf32' or 'cs16'. (Default: cs16)", NULL, 0, 0),
    OPT_INTEGER(
        0, "airspy-serial", (int *)&s_airspy_config.serial_number,
        "Select device by serial number (hex, e.g., 0x123456789ABCDEF0).", NULL,
        0, 0),
    OPT_BOOLEAN(0, "airspy-packing", &s_airspy_config.packing_enabled,
                "Enable bit-packing mode (12-bit samples).", NULL, 0, 0),
};

static const struct argparse_option *input_airspy_get_cli_options(int *count) {
  *count =
      sizeof(input_airspy_cli_options) / sizeof(input_airspy_cli_options[0]);
  return input_airspy_cli_options;
}
...
static int input_airspy_buffered_stream_callback(airspy_transfer *transfer);
...
static bool input_airspy_validate_options(AppContext *app) {
  ...
  if (s_airspy_config.gain_mode) {
    ...
    if (strcasecmp(s_airspy_config.gain_mode, "linearity") != 0 &&
      ...
      log_error("Invalid --airspy-gain-mode '%s'. Must be 'linearity', "
                "'sensitivity', or 'manual'.",
                s_airspy_config.gain_mode);
      return false;
    }
  }
  ...
  if (s_airspy_config.airspy_gain_value_arg != AIRSPY_DEFAULT_GAIN_VALUE) {
    if (s_airspy_config.airspy_gain_value_arg < 0 ||
        s_airspy_config.airspy_gain_value_arg > 21) {
      log_error("Invalid --airspy-gain-value %d. Must be between 0 and 21.",
                s_airspy_config.airspy_gain_value_arg);
      return false;
    }
  ...
  if (s_airspy_config.airspy_lna_gain_arg != AIRSPY_DEFAULT_LNA_GAIN) {
    if (s_airspy_config.airspy_lna_gain_arg < 0 ||
        s_airspy_config.airspy_lna_gain_arg > 14) {
      log_error("Invalid --airspy-lna-gain %d. Must be between 0 and 14.",
                s_airspy_config.airspy_lna_gain_arg);
      return false;
    }
  ...
  if (s_airspy_config.airspy_mixer_gain_arg != AIRSPY_DEFAULT_MIXER_GAIN) {
    if (s_airspy_config.airspy_mixer_gain_arg < 0 ||
        s_airspy_config.airspy_mixer_gain_arg > 15) {
      log_error("Invalid --airspy-mixer-gain %d. Must be between 0 and 15.",
                s_airspy_config.airspy_mixer_gain_arg);
      return false;
    }
  ...
  if (s_airspy_config.airspy_vga_gain_arg != AIRSPY_DEFAULT_VGA_GAIN) {
    if (s_airspy_config.airspy_vga_gain_arg < 0 ||
        s_airspy_config.airspy_vga_gain_arg > 15) {
      log_error("Invalid --airspy-vga-gain %d. Must be between 0 and 15.",
                s_airspy_config.airspy_vga_gain_arg);
      return false;
    }
  ...
  if (s_airspy_config.sample_format) {
    ...
    if (strcasecmp(s_airspy_config.sample_format, "cf32") != 0 &&
      ...
      log_error(
          "Invalid --airspy-sample-format '%s'. Must be 'cf32' or 'cs16'.",
          s_airspy_config.sample_format);
      return false;
    }
  }
  ...
  if (config->sdr_general.sample_rate_provided) {
    ...
    const size_t NUM_RATES =
        sizeof(VALID_AIRSPY_RATES) / sizeof(VALID_AIRSPY_RATES[0]);
    ...
    if (!rate_found) {
      ...
      log_error(
          "Valid sample rates for Airspy Mini: 3000000, 6000000, 10000000");
  ...
  if ((s_airspy_config.lna_gain_provided ||
       s_airspy_config.mixer_gain_provided ||
       s_airspy_config.vga_gain_provided) &&
      s_airspy_config.gain_mode_provided &&
      strcasecmp(s_airspy_config.gain_mode, "manual") != 0) {
    log_error(
        "Manual gain options (--airspy-lna-gain, --airspy-mixer-gain, "
        "--airspy-vga-gain) can only be used with --airspy-gain-mode manual.");
    return false;
  }

  // Validate that gain-value is only used with linearity/sensitivity modes
  if (s_airspy_config.gain_value_provided &&
      s_airspy_config.gain_mode_provided &&
      strcasecmp(s_airspy_config.gain_mode, "manual") == 0) {
    log_error("Option --airspy-gain-value cannot be used with "
              "--airspy-gain-mode manual. Use --airspy-lna-gain, "
              "--airspy-mixer-gain, and --airspy-vga-gain instead.");
    return false;
  }
...
static int input_airspy_buffered_stream_callback(airspy_transfer *transfer) {
  ...
  case AIRSPY_SAMPLE_RAW:
    // We force INT16_IQ mode during initialization, so we should not see RAW
    // here. If we do, it implies the library failed to unpack or configuration
    // is wrong.
    request_forceful_shutdown(
        "Airspy unexpected RAW sample type. Library unpacking config error?",
        app);
    return -1;
...
static void input_airspy_get_summary_info(const ModuleContext *context,
  ...
  const char *source_name = (private_data && private_data->board_name)
  ...
  if (s_airspy_config.gain_mode_provided) {
    if (strcasecmp(s_airspy_config.gain_mode, "linearity") == 0) {
      utility_add_summary_item(info, "Gain Mode", "Linearity (Level: %d)",
                               s_airspy_config.gain_value);
    } else if (strcasecmp(s_airspy_config.gain_mode, "sensitivity") == 0) {
      utility_add_summary_item(info, "Gain Mode", "Sensitivity (Level: %d)",
                               s_airspy_config.gain_value);
    } else if (strcasecmp(s_airspy_config.gain_mode, "manual") == 0) {
      utility_add_summary_item(info, "Gain Mode", "Manual");
      utility_add_summary_item(info, "LNA Gain", "%d",
                               s_airspy_config.lna_gain);
      utility_add_summary_item(info, "Mixer Gain", "%d",
                               s_airspy_config.mixer_gain);
      utility_add_summary_item(info, "VGA Gain", "%d",
                               s_airspy_config.vga_gain);
    }
  } else {
...
static bool input_airspy_initialize(ModuleContext *context) {
  const AppConfig *config = context->config;
  AppContext *app = context->app;
  int result;
  bool success = false;

  AirspyContext *private_data = (AirspyContext *)mem_arena_alloc(
      &app->process_chain.setup_arena, sizeof(AirspyContext), true);
  if (!private_data) {
    return false;
  }
  ...
  if (result != AIRSPY_SUCCESS) {
    log_error("airspy_init() failed: %s (%d)", airspy_error_name(result),
              result);
    goto cleanup;
  }
  ...
  if (result != AIRSPY_SUCCESS) {
    log_error("airspy_open() failed: %s (%d)", airspy_error_name(result),
              result);
    private_data->dev = NULL;
    goto cleanup;
  }
  ...
  log_info("Using Airspy device: %s (S/N: 0x%08X%08X%08X%08X)",
           private_data->board_name, s.serial_no[0], s.serial_no[1],
           s.serial_no[2], s.serial_no[3]);

  // Get and validate supported sample rates
  uint32_t num_rates = 0;
  result = airspy_get_samplerates(private_data->dev, &num_rates, 0);
  if (result != AIRSPY_SUCCESS) {
    log_error("Failed to get number of supported sample rates: %s (%d)",
  ...
  if (result != AIRSPY_SUCCESS) {
    log_error("Failed to get supported sample rates: %s (%d)",
              airspy_error_name(result), result);
    goto cleanup;
  }
  ...
  if (result != AIRSPY_SUCCESS) {
    log_error("airspy_set_samplerate() failed: %s (%d)",
              airspy_error_name(result), result);
    goto cleanup;
  }

  // Set frequency
  result = airspy_set_freq(private_data->dev,
                           (uint32_t)config->sdr_general.rf_freq_hz);
  if (result != AIRSPY_SUCCESS) {
    log_error("airspy_set_freq() failed: %s (%d)", airspy_error_name(result),
              result);
    goto cleanup;
  }
  ...
  if (s_airspy_config.packing_enabled) {
    ...
    if (result != AIRSPY_SUCCESS) {
      log_warn("Failed to enable packing mode: %s (%d)",
               airspy_error_name(result), result);
    } else {
      ...
      if (s_airspy_config.sample_format_provided &&
        ...
        log_warn("Airspy Packing enabled: Overriding sample format to CS16 "
                 "(Packed mode does not support Float).");
      }
      app->module.input_format = CS16;

      // When packing is enabled, we MUST force the sample type to INT16_IQ.
      // This tells the library to unpack the raw 12-bit data into 16-bit
      // integers for us.
      private_data->sample_type = AIRSPY_SAMPLE_INT16_IQ;
    }
  ...
  // Set sample type
  // This needs to happen regardless of packing status to ensure the library
  // knows what output format we expect (INT16_IQ or FLOAT32_IQ).
  result = airspy_set_sample_type(private_data->dev, private_data->sample_type);
  if (result != AIRSPY_SUCCESS) {
    log_error("airspy_set_sample_type() failed: %s (%d)",
              airspy_error_name(result), result);
    goto cleanup;
  }

  // Configure gain
  bool any_gain_option_specified =
      s_airspy_config.gain_mode_provided || s_airspy_config.lna_gain_provided ||
      s_airspy_config.mixer_gain_provided || s_airspy_config.vga_gain_provided;
  ...
  if (!any_gain_option_specified) {
    ...
    if (result != AIRSPY_SUCCESS) {
      log_error("airspy_set_lna_agc() failed: %s (%d)",
                airspy_error_name(result), result);
      goto cleanup;
    }
    result = airspy_set_mixer_agc(private_data->dev, 1);
    if (result != AIRSPY_SUCCESS) {
      log_error("airspy_set_mixer_agc() failed: %s (%d)",
                airspy_error_name(result), result);
      goto cleanup;
    }
  } else {
    ...
    if (s_airspy_config.gain_mode_provided) {
      if (strcasecmp(s_airspy_config.gain_mode, "linearity") == 0) {
        result = airspy_set_linearity_gain(private_data->dev,
                                           s_airspy_config.gain_value);
        if (result != AIRSPY_SUCCESS) {
          log_error("airspy_set_linearity_gain() failed: %s (%d)",
                    airspy_error_name(result), result);
          goto cleanup;
        }
      } else if (strcasecmp(s_airspy_config.gain_mode, "sensitivity") == 0) {
        result = airspy_set_sensitivity_gain(private_data->dev,
                                             s_airspy_config.gain_value);
        if (result != AIRSPY_SUCCESS) {
          log_error("airspy_set_sensitivity_gain() failed: %s (%d)",
                    airspy_error_name(result), result);
          goto cleanup;
        }
      } else if (strcasecmp(s_airspy_config.gain_mode, "manual") == 0) {
        result =
            airspy_set_lna_gain(private_data->dev, s_airspy_config.lna_gain);
        if (result != AIRSPY_SUCCESS) {
          log_error("airspy_set_lna_gain() failed: %s (%d)",
                    airspy_error_name(result), result);
          goto cleanup;
        }
        result = airspy_set_mixer_gain(private_data->dev,
                                       s_airspy_config.mixer_gain);
        if (result != AIRSPY_SUCCESS) {
          log_error("airspy_set_mixer_gain() failed: %s (%d)",
                    airspy_error_name(result), result);
          goto cleanup;
        }
        result =
            airspy_set_vga_gain(private_data->dev, s_airspy_config.vga_gain);
        if (result != AIRSPY_SUCCESS) {
          log_error("airspy_set_vga_gain() failed: %s (%d)",
                    airspy_error_name(result), result);
          goto cleanup;
        }
      }
  ...
  if (config->sdr_general.bias_t_enable) {
    ...
    if (result != AIRSPY_SUCCESS) {
      log_error("airspy_set_rf_bias() failed: %s (%d)",
                airspy_error_name(result), result);
      goto cleanup;
    }
  }
...
cleanup:
  if (!success) {
    // Cleanup will be handled by input_airspy_cleanup()
  }
  return success;
}

static void input_airspy_stop_sample_queue_push(ModuleContext *context);

static void *input_airspy_push_samples_to_queue(ModuleContext *context,
  ...
  callback_fn = input_airspy_buffered_stream_callback;

  result = airspy_start_rx(private_data->dev, callback_fn, app);
  if (result != AIRSPY_SUCCESS) {
    char error_buf[256];
    snprintf(error_buf, sizeof(error_buf), "airspy_start_rx() failed: %s (%d)",
             airspy_error_name(result), result);
  ...
  if (!is_shutdown_requested()) {
    input_airspy_stop_sample_queue_push(context);
  }
...
static void input_airspy_stop_sample_queue_push(ModuleContext *context) {
  ...
  if (private_data) {
    ...
    if (private_data && private_data->dev &&
        airspy_is_streaming(private_data->dev) == AIRSPY_TRUE) {
      log_debug("Stopping Airspy stream...");
      int result = airspy_stop_rx(private_data->dev);
      if (result != AIRSPY_SUCCESS) {
        log_error("Failed to stop Airspy RX: %s (%d)",
                  airspy_error_name(result), result);
      }
    }
...
static void input_airspy_cleanup(ModuleContext *context) {
  AppContext *app = context->app;
  if (app->module.input_private_data) {
    AirspyContext *private_data =
        (AirspyContext *)app->module.input_private_data;
    pthread_mutex_lock(&private_data->driver_mutex);
    if (private_data->dev) {
...
static InputModuleInterface s_input_airspy_api = {
    .initialize = input_airspy_initialize,
    .push_samples_to_queue = input_airspy_push_samples_to_queue,
    .stop_sample_queue_push = input_airspy_stop_sample_queue_push,
    .cleanup = input_airspy_cleanup,
    .get_summary_info = input_airspy_get_summary_info,
    .validate_options = input_airspy_validate_options,
