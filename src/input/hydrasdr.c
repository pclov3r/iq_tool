...
input_hydrasdr_get_cli_options(int *count) {
  *count = sizeof(input_hydrasdr_cli_options) /
           sizeof(input_hydrasdr_cli_options[0]);
...
static bool input_hydrasdr_validate_options(AppContext *app) {
  ...
  if (s_hydrasdr_config.gain_mode) {
    ...
    if (strcasecmp(s_hydrasdr_config.gain_mode, "linearity") != 0 &&
      ...
      log_error("Invalid --hydrasdr-gain-mode '%s'. Must be 'linearity', "
                "'sensitivity', or 'manual'.",
                s_hydrasdr_config.gain_mode);
      return false;
    }
  }

  // Gain Value Validation
  if (s_hydrasdr_config.hydrasdr_gain_value_arg !=
      HYDRASDR_DEFAULT_GAIN_VALUE) {
    if (s_hydrasdr_config.hydrasdr_gain_value_arg < 0 ||
        s_hydrasdr_config.hydrasdr_gain_value_arg > 21) {
      log_error("Invalid --hydrasdr-gain-value %d. Must be between 0 and 21.",
                s_hydrasdr_config.hydrasdr_gain_value_arg);
      return false;
    }
  ...
  if (s_hydrasdr_config.hydrasdr_lna_gain_arg != HYDRASDR_DEFAULT_LNA_GAIN) {
    if (s_hydrasdr_config.hydrasdr_lna_gain_arg < 0 ||
        s_hydrasdr_config.hydrasdr_lna_gain_arg > 14) {
      log_error("Invalid --hydrasdr-lna-gain %d. Must be between 0 and 14.",
  ...
  if (s_hydrasdr_config.hydrasdr_mixer_gain_arg !=
      HYDRASDR_DEFAULT_MIXER_GAIN) {
    if (s_hydrasdr_config.hydrasdr_mixer_gain_arg < 0 ||
        s_hydrasdr_config.hydrasdr_mixer_gain_arg > 15) {
      log_error("Invalid --hydrasdr-mixer-gain %d. Must be between 0 and 15.",
  ...
  if (s_hydrasdr_config.hydrasdr_vga_gain_arg != HYDRASDR_DEFAULT_VGA_GAIN) {
    if (s_hydrasdr_config.hydrasdr_vga_gain_arg < 0 ||
        s_hydrasdr_config.hydrasdr_vga_gain_arg > 15) {
      log_error("Invalid --hydrasdr-vga-gain %d. Must be between 0 and 15.",
  ...
  if (s_hydrasdr_config.sample_format) {
    ...
    if (strcasecmp(s_hydrasdr_config.sample_format, "cf32") != 0 &&
      ...
      log_error(
          "Invalid --hydrasdr-sample-format '%s'. Must be 'cf32' or 'cs16'.",
          s_hydrasdr_config.sample_format);
      return false;
    }
  }
  ...
  if ((s_hydrasdr_config.lna_gain_provided ||
       s_hydrasdr_config.mixer_gain_provided ||
       s_hydrasdr_config.vga_gain_provided) &&
      s_hydrasdr_config.gain_mode_provided &&
      strcasecmp(s_hydrasdr_config.gain_mode, "manual") != 0) {
    log_error("Manual gain options (--hydrasdr-lna-gain, "
              "--hydrasdr-mixer-gain, --hydrasdr-vga-gain) can only be used "
              "with --hydrasdr-gain-mode manual.");
    return false;
  }

  // Validate that gain-value is only used with linearity/sensitivity modes
  if (s_hydrasdr_config.gain_value_provided &&
      s_hydrasdr_config.gain_mode_provided &&
      strcasecmp(s_hydrasdr_config.gain_mode, "manual") == 0) {
    log_error("Option --hydrasdr-gain-value cannot be used with "
              "--hydrasdr-gain-mode manual. Use --hydrasdr-lna-gain, "
              "--hydrasdr-mixer-gain, and --hydrasdr-vga-gain instead.");
    return false;
  }
...
input_hydrasdr_buffered_stream_callback(hydrasdr_transfer *transfer) {
  ...
  case HYDRASDR_SAMPLE_UINT16_REAL:
    ...
    request_forceful_shutdown("HydraSDR real sample format not supported.",
                              app);
    return -1;
  ...
  case HYDRASDR_SAMPLE_RAW:
    ...
    request_forceful_shutdown(
        "HydraSDR unexpected RAW sample type. Library unpacking config error?",
        app);
    return -1;
...
static bool input_hydrasdr_initialize(ModuleContext *context) {
  ...
  HydraSDRContext *private_data = (HydraSDRContext *)mem_arena_alloc(
      &app->process_chain.setup_arena, sizeof(HydraSDRContext), true);
  if (!private_data) {
    return false;
  }
  ...
  if (s_hydrasdr_config.serial_provided) {
    result =
        hydrasdr_open_sn(&private_data->dev, s_hydrasdr_config.serial_number);
  } else {
    result = hydrasdr_open(&private_data->dev);
  }

  if (result != HYDRASDR_SUCCESS) {
    log_error("hydrasdr_open() failed: %s (%d)", hydrasdr_error_name(result),
              result);
    private_data->dev = NULL;
    goto cleanup;
  }
  ...
  if (result == HYDRASDR_SUCCESS) {
    ...
    char *safe_name = (char *)mem_arena_alloc(&app->process_chain.setup_arena,
                                              name_len + 1, false);
  ...
  if (!rate_supported) {
    log_error("Requested sample rate %.15g Hz is not supported.",
  ...
  if (s_hydrasdr_config.packing_enabled) {
    ...
    if (result != HYDRASDR_SUCCESS) {
      ...
      if (s_hydrasdr_config.sample_format_provided &&
        ...
        log_warn("HydraSDR Packing enabled: Overriding sample format to CS16 "
  ...
  if (config->sdr_general.bias_t_enable) {
    ...
    if (result != HYDRASDR_SUCCESS) {
      log_error("hydrasdr_set_rf_bias() failed: %s (%d)",
                hydrasdr_error_name(result), result);
      goto cleanup;
    }
  }
...
static void *input_hydrasdr_push_samples_to_queue(ModuleContext *context,
  ...
  uint32_t *samplerates = (uint32_t *)mem_arena_alloc(
      &app->process_chain.setup_arena, sr_count * sizeof(uint32_t), false);
  if (samplerates) {
  ...
  if (!any_gain_option_specified) {
    ...
    if (info.features & HYDRASDR_CAP_LNA_AGC)
      hydrasdr_set_gain(private_data->dev, HYDRASDR_GAIN_TYPE_LNA_AGC, 0);
    if (info.features & HYDRASDR_CAP_MIXER_AGC)
      hydrasdr_set_gain(private_data->dev, HYDRASDR_GAIN_TYPE_MIXER_AGC, 0);

    // Apply gain settings
    if (s_hydrasdr_config.gain_mode_provided) {
      if (strcasecmp(s_hydrasdr_config.gain_mode, "linearity") == 0) {
        if (info.features & HYDRASDR_CAP_LINEARITY_GAIN) {
          hydrasdr_set_gain(private_data->dev, HYDRASDR_GAIN_TYPE_LINEARITY,
                            s_hydrasdr_config.gain_value);
        } else {
          request_forceful_shutdown(
              "Linearity gain mode is not supported by this hardware.", app);
          return NULL;
        }
      } else if (strcasecmp(s_hydrasdr_config.gain_mode, "sensitivity") == 0) {
        if (info.features & HYDRASDR_CAP_SENSITIVITY_GAIN) {
          hydrasdr_set_gain(private_data->dev, HYDRASDR_GAIN_TYPE_SENSITIVITY,
                            s_hydrasdr_config.gain_value);
        } else {
          request_forceful_shutdown(
              "Sensitivity gain mode is not supported by this hardware.", app);
          return NULL;
        }
      } else if (strcasecmp(s_hydrasdr_config.gain_mode, "manual") == 0) {
        if (info.features & HYDRASDR_CAP_LNA_GAIN)
          hydrasdr_set_gain(private_data->dev, HYDRASDR_GAIN_TYPE_LNA,
                            s_hydrasdr_config.lna_gain);
        if (info.features & HYDRASDR_CAP_MIXER_GAIN)
          hydrasdr_set_gain(private_data->dev, HYDRASDR_GAIN_TYPE_MIXER,
                            s_hydrasdr_config.mixer_gain);
        if (info.features & HYDRASDR_CAP_VGA_GAIN)
          hydrasdr_set_gain(private_data->dev, HYDRASDR_GAIN_TYPE_VGA,
                            s_hydrasdr_config.vga_gain);
      }
    }
  }
...
static void input_hydrasdr_stop_sample_queue_push(ModuleContext *context) {
  ...
  if (private_data) {
    ...
    if (private_data->dev &&
      ...
      if (result != HYDRASDR_SUCCESS) {
        log_error("Failed to stop HydraSDR RX: %s (%d)",
                  hydrasdr_error_name(result), result);
      }
    }
