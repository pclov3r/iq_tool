...
static bool validate_and_process_args(AppContext *app, int non_opt_argc,
                                      const char **non_opt_argv);
static int version_cb(struct argparse *self,
                      const struct argparse_option *option);
static int build_cli_options(struct argparse_option *options_buffer,
...
static int preset_flag_warning_cb(struct argparse *self,
                                  const struct argparse_option *option) {
  (void)self;
  log_error(
      "'--%s' is not a valid flag. To load the '%s' preset, use '--preset %s'.",
      option->long_name, option->long_name, option->long_name);
  exit(EXIT_FAILURE);
  return 0;
}

void cli_print_usage(const char *prog_name, AppConfig *config,
                     MemoryArena *arena) {
  (void)prog_name;
...
static int build_cli_options(struct argparse_option *options_buffer,
  ...
  struct argparse_option generic_options[] = {
      ...
      OPT_STRING('i', "input", &config->input.type_name,
                 "Specifies the input module.", NULL, 0, 0),
      OPT_STRING('o', "output", &config->output.module_name,
                 "Specifies the output module and optional file path", NULL, 0,
                 0),

      OPT_GROUP("Audio Output Options"),
      OPT_STRING(0, "audio-writer", &config->audio.path_arg,
                 "Save demodulated audio to a WAV file.", NULL, 0, 0),
      OPT_BOOLEAN(0, "audio-writer-rf64", &config->audio.writer_rf64,
                  "Use RF64 format for the audio writer (supports >4GB files).",
                  NULL, 0, 0),
      OPT_BOOLEAN(0, "mute-audio", &config->audio.mute,
                  "Disable speaker playback", NULL, 0, 0),

      OPT_GROUP("Input Processing Options"),
      OPT_FLOAT(0, "input-gain-multiplier", &config->dsp.input_gain,
                "Apply a linear gain multiplier to INPUT samples (before "
                "processing).",
                NULL, 0, 0),

      OPT_GROUP("Baseband Processing Options"),
      OPT_DOUBLE(
          0, "baseband-sample-rate", &config->baseband_sample_rate.user_arg,
          "Baseband sample rate feeding into a demodulator in Hz.", NULL, 0, 0),
      OPT_STRING(
          0, "baseband-sample-format",
          &config->baseband_sample_format.format_str,
          "Baseband sample format feeding into a demodulator {cf32|cs16|...}.",
          NULL, 0, 0),
      OPT_FLOAT(0, "baseband-gain-multiplier", &config->dsp.baseband_gain,
                "Apply a linear gain multiplier to baseband samples before "
                "demodulation.",
                NULL, 0, 0),

      OPT_GROUP("I/Q Output Options"),
      OPT_DOUBLE(0, "output-sample-rate", &config->output_sample_rate.user_arg,
                 "Output sample rate in Hz.", NULL, 0, 0),
      OPT_STRING(0, "output-sample-format", &config->output.sample_format_str,
                 "Sample format for output data {cs8|cu8|cs16|...}.", NULL, 0,
                 0),
      OPT_FLOAT(
          0, "output-gain-multiplier", &config->dsp.output_gain,
      ...
      OPT_BOOLEAN(
          ...
          NULL, 0, 0),
      OPT_STRING(0, "preset", &config->preset_name,
                 "Use a preset for a common target.", NULL, 0, 0),
  };

  struct argparse_option sdr_general_options[] = {
      OPT_GROUP("SDR General Options"),
      OPT_DOUBLE(0, "sdr-rf-freq", &config->sdr_general.rf_freq_hz_arg,
                 "(Required for SDR) Tuner center frequency in Hz", NULL, 0, 0),
      OPT_DOUBLE(
          0, "sdr-freq-offset", &config->sdr_general.frequency_offset_arg,
          "Frequency offset in Hz (e.g. 125e6 for HamItUp).", NULL, 0, 0),
      OPT_DOUBLE(0, "sdr-sample-rate", &config->sdr_general.sample_rate_hz_arg,
                 "Set sample rate in Hz. (Device-specific default)", NULL, 0,
                 0),
      OPT_BOOLEAN(0, "sdr-bias-t", &config->sdr_general.bias_t_enable,
                  "(Optional) Enable Bias-T power.", NULL, 0, 0),
  };
...
bool cli_parse(int argc, char *argv[], AppContext *app) {
  ...
  struct argparse_option all_options[MAX_TOTAL_OPTIONS];
  if (build_cli_options(all_options, MAX_TOTAL_OPTIONS, config, arena,
  ...
  if (!validate_and_process_args(app, non_opt_argc, argparse.out))
    return false;
  ...
  for (int i = 0; i < config->num_presets; i++) {
    if (strcasecmp(config->preset_name, config->presets[i].name) == 0) {
      ...
      if (!config->output.sample_format_str)
        config->output.sample_format_str = p->output_sample_format;
      if (!config->baseband_sample_format.format_str)
        config->baseband_sample_format.format_str = p->baseband_sample_format;
      ...
      if (p->baseband_gain_provided && config->dsp.baseband_gain == 1.0f)
        config->dsp.baseband_gain = p->baseband_gain;
      ...
      if (p->output_agc_enable_provided && !config->dsp.output_agc.enable)
        config->dsp.output_agc.enable = p->output_agc_enable;
      if (p->output_agc_target_provided &&
          config->dsp.output_agc.target_level_arg == 0.0f)
        config->dsp.output_agc.target_level_arg = p->output_agc_target;
      if (p->baseband_agc_enable_provided && !config->dsp.baseband_agc.enable)
        config->dsp.baseband_agc.enable = p->baseband_agc_enable;
      if (p->baseband_agc_target_provided &&
          config->dsp.baseband_agc.target_level_arg == 0.0f)
        config->dsp.baseband_agc.target_level_arg = p->baseband_agc_target;
      if (p->lowpass_cutoff_hz_provided &&
          config->dsp.filter.args.lowpass[0] == 0.0f)
        config->dsp.filter.args.lowpass[0] = p->lowpass_cutoff_hz;
      if (p->highpass_cutoff_hz_provided &&
          config->dsp.filter.args.highpass[0] == 0.0f)
      ...
      if (p->transition_width_hz_provided &&
          config->dsp.filter.args.transition_width == 0.0f)
      ...
      if (p->default_filter_attenuation_db_provided &&
          config->dsp.filter.args.attenuation == 0.0f)
        config->dsp.filter.args.attenuation = p->default_filter_attenuation_db;
      if (p->filter_type_str_provided && !config->dsp.filter.args.type_str)
        config->dsp.filter.args.type_str = p->filter_type_str;
      return;
    }
  }
  log_error("Unknown preset '%s'. Check '%s' or --help for available presets.",
            config->preset_name, PRESETS_FILENAME);
  exit(EXIT_FAILURE);
}
...
static bool resolve_file_paths(AppConfig *config, MemoryArena *arena) {
  ...
  if (config->input.path_arg) {
    ...
    if (realpath(config->input.path_arg, resolved_input_path) == NULL) {
      log_error("Input file not found or path is invalid: %s (%s)",
  ...
  if (config->output.path_arg) {
    char *path_copy_for_dirname =
        mem_arena_alloc(arena, strlen(config->output.path_arg) + 1, false);
  ...
  if (config->audio.path_arg) {
    char *path_copy_for_dirname =
        mem_arena_alloc(arena, strlen(config->audio.path_arg) + 1, false);
    char *path_copy_for_basename =
        mem_arena_alloc(arena, strlen(config->audio.path_arg) + 1, false);
    if (!path_copy_for_dirname || !path_copy_for_basename)
      return false;
    ...
    if (realpath(dir, resolved_dir_path) == NULL) {
      log_error(
          "Audio writer directory does not exist or path is invalid: %s (%s)",
          dir, strerror(errno));
      return false;
    }
    size_t final_length = strlen(resolved_dir_path) + 1 + strlen(base) + 1;
...
static bool validate_and_process_args(AppContext *app, int non_opt_argc,
  ...
  InputModuleInterface *selected_module_api =
      (InputModuleInterface *)selected_input_module->api;
  if (!selected_module_api)
    return false;

  if (selected_input_module->requires_input_path) {
    if (non_opt_argc < 1) {
      log_error("Missing <in_file> argument for input type '%s'.",
  ...
  if (selected_output_module->requires_output_path) {
    if (non_opt_argc < 1) {
      log_error("Missing <out_file> argument for '--output %s'.",
  ...
  if (config->output_sample_rate.user_arg > 0.0f) {
    config->output_sample_rate.rate_hz =
        (double)config->output_sample_rate.user_arg;
    config->output_sample_rate.provided = true;
  }
  if (config->baseband_sample_rate.user_arg > 0.0f) {
    config->baseband_sample_rate.rate_hz =
        (double)config->baseband_sample_rate.user_arg;
    config->baseband_sample_rate.provided = true;
  }
  ...
  if (config->sdr_general.frequency_offset_arg != 0.0f) {
    config->sdr_general.frequency_offset_hz =
        config->sdr_general.frequency_offset_arg;
  }
  ...
  if (!resolve_file_paths(config, arena))
    return false;
  ...
  for (int i = 0; i < num_modules; ++i) {
    if (modules[i].type == MODULE_TYPE_DSP) {
      const DspModuleInterface *dsp_mod =
          (const DspModuleInterface *)modules[i].api;
      if (dsp_mod && dsp_mod->validate_options) {
        if (!dsp_mod->validate_options(app)) {
          return false;
        }
      }
    }
  ...
  if (selected_module_api->validate_options &&
      !selected_module_api->validate_options(app))
    return false;
  if (selected_module_api->validate_generic_options &&
      !selected_module_api->validate_generic_options(config))
    return false;
  ...
  if (out_mod_val) {
    ...
    if (out_api && out_api->validate_options) {
      if (!out_api->validate_options(app))
        return false;
    }
  }
