...
static bool _has_space_for(RingBuffer *buffer, size_t bytes_needed) {
  ...
  // Ring buffer implementation typically reserves 1 byte to distinguish full
  // from empty
...
int64_t packet_serializer_read_packet(RingBuffer *buffer,
  ...
  *is_reset_event = false;
  ...
  if (state->samples_remaining_in_packet == 0) {
    ...
    if (bytes_read == 0)
      return 0; // End of Stream (Normal)

    // SANITY CHECK: Fail Fast.
    if (bytes_read < sizeof(header) || header.magic != IQPK_MAGIC) {
      log_error("Stream Sync Error: Magic number mismatch! Expected 0x%08X, "
                "got 0x%08X. Buffer overrun likely.",
  ...
  // Validate format
  const SampleFormatInfo *pkt_fmt_info =
      get_format_info_by_enum(state->current_packet_format);
