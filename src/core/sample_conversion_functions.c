...
/*
 * sample_conversion_functions.c: Functions for converting between various I/Q
 * sample formats.
 *
...
bool sample_convert_block_to_cf32(const void *restrict input_buffer,
                                  ...
                                  float gain) {
  // Add robustness checks for debug builds. These compile to nothing in release
  // builds.
...
#define CS24_IN_STEP(out_idx, byte_offset)                                     \
  do {                                                                         \
    ...
    for (; i < num_frames; ++i) {
      int32_t i_val = (int32_t)in_ptr[0] | ((int32_t)in_ptr[1] << 8) |
                      ((int32_t)in_ptr[2] << 16);
      int32_t q_val = (int32_t)in_ptr[3] | ((int32_t)in_ptr[4] << 8) |
                      ((int32_t)in_ptr[5] << 16);
      i_val = (i_val << 8) >> 8;
      q_val = (q_val << 8) >> 8;
...
#undef S24_IN_STEP
  ...
  case CS32: {
    // This case is handled separately to maintain double precision during
    // normalization.
    const int32_t *in = (const int32_t *)input_buffer;
    // Use double for intermediate precision to avoid losing info from the
    // 32-bit int.
    const double normalizer = 1.0 / 2147483648.0;
    for (size_t i = 0; i < num_frames; ++i) {
  ...
  case CF32: {
    // This case is a direct copy and gain multiplication, no normalization
    // needed.
