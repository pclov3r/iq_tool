...
Queue* queue_create(size_t capacity) {
    if (capacity == 0) {
        fprintf(stderr, "Error: Queue capacity cannot be zero.\n");
        return NULL;
    }
    ...
    if ((ret = pthread_cond_init(&q->not_empty_cond, NULL)) != 0) {
        fprintf(stderr, "Error: pthread_cond_init (not_empty) failed: %s\n", strerror(ret));
    ...
    if ((ret = pthread_cond_init(&q->not_full_cond, NULL)) != 0) {
        fprintf(stderr, "Error: pthread_cond_init (not_full) failed: %s\n", strerror(ret));
