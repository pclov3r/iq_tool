...
void utils_wait_for_ring_buffer_drain(RingBuffer* rb, int poll_interval_ms, int stall_timeout_ms, int hardware_padding_ms) {
    ...
    // Safety default to prevent infinite loops or divide-by-zero
    ...
    while (true) {
        size_t curr_size = ring_buffer_get_size(rb);

        // 1. Success: Buffer is empty
        ...
        if (curr_size == last_size) {
            ...
            if (stall_count > max_stall_iterations) {
                // Buffer is stuck (likely smaller than callback request size)
                break;
            }
        } else {
            stall_count = 0;
