...
bool set_stdout_binary(void) {
    if (_setmode(_fileno(stdout), _O_BINARY) == -1) {
        fprintf(stderr, "\nError: Failed to set stdout to binary mode: %s\n", strerror(errno));
        return false;
    }
    return true;
}
...
void print_win_error(const char* context, DWORD error_code) {
    ...
    if (messageBuffer && size > 0) {
        fwprintf(stderr, L"\nError: %hs failed. Code: %lu, Message: %ls\n", context, error_code, messageBuffer);
        LocalFree(messageBuffer);
    } else {
        fprintf(stderr, "\nError: %s failed. Code: %lu (Could not retrieve error message)\n", context, error_code);
    }
}
