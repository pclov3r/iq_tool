/**
 * @file common_types.h
 * @brief Defines fundamental, low-level data types and enums for the
 * application.
 */
...
_Static_assert(sizeof(ComplexFloat) == 8,
               "ComplexFloat must be exactly 8 bytes (2x 32-bit floats)");
...
typedef enum { PAYLOAD_IQ, PAYLOAD_AUDIO } OutputPayload;
...
typedef enum {
  PROCESS_CHAIN_MODE_ASYNCHRONOUS_PUSH,
