 ...
 * @brief Defines the unified registry and lookup functions for I/Q sample
 * formats.
 *
 ...
 * It replaces scattered switch statements with highly optimized O(1) jump
 * tables.
 */
...
typedef struct {
  ...
  const char
      *name_str; ///< The short command-line identifier (e.g., "cs16", "cf32").
  const char
      *description_str; ///< A human-readable description for UI and logging.
  size_t bytes_per_iq_sample; ///< Total memory footprint in bytes for one
                              ///< complete I/Q pair.
