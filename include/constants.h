...
// Defines the base number of samples to read from the input source in each chunk.
// This is NOT the full processing buffer size, which is calculated dynamically at runtime.
#define PIPELINE_INPUT_CHUNK_SIZE_SAMPLES 16384

// Defines the number of chunks to allocate for the pipeline.
#define NUM_BUFFERS                     512

// A safety margin for the resampler's output calculation.
#define RESAMPLER_OUTPUT_SAFETY_MARGIN  128

// A hard upper limit on the FFT filter block size to prevent unreasonable memory
// allocation if filter parameters result in a huge number of taps.
