 ...
 * @brief Defines a unified audio output interface, abstracting OS sound drivers
 * and buffers.
 */
...
AudioOutputContext *audio_output_create(AppContext *app, int sample_rate,
                                        int channels, size_t buffer_size_bytes);
 ...
 * @param mode The current process_chain mode (File vs. Live SDR).
...
/**
 * @brief Blocks until the internal audio buffer drains (useful at
 * End-of-Stream).
 */
