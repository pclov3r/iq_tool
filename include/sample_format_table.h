...
// --- The Single Source of Truth for all Sample Format Properties ---
typedef struct {
...
static const SampleFormatInfo MASTER_FORMAT_TABLE[] = {
    // Enum,     "name",    "Description",                        Bytes, dBm Offset,              Attenuation
...
static inline const SampleFormatInfo* get_format_info_by_name(const char* name) {
    if (!name) return NULL;
    for (int i = 0; i < NUM_MASTER_FORMATS; ++i) {
        if (strcasecmp(name, MASTER_FORMAT_TABLE[i].name_str) == 0) {
            return &MASTER_FORMAT_TABLE[i];
        }
    }
    return NULL;
