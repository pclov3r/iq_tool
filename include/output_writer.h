 ...
 * container format. It provides a generic interface (`OutputWriterOps`) that can be
 * implemented by different "writers" (e.g., for raw binary files or WAV files).
 *
 * The output_writer_init() function selects the correct implementation based on the
...
struct AppResources;
struct MemoryArena;
struct OutputWriterContext;

// --- Type Definitions ---

/**
 * @struct OutputWriterOps
 ...
 */
typedef struct OutputWriterOps {
    /**
     ...
     */
    bool (*open)(struct OutputWriterContext* ctx, const struct AppConfig* config, struct AppResources* resources, struct MemoryArena* arena);
     ...
     */
    size_t (*write)(struct OutputWriterContext* ctx, const void* buffer, size_t bytes_to_write);

    /**
     * @brief Closes the output file or stream and releases resources.
     * @param ctx Pointer to the context to be cleaned up.
     */
    void (*close)(struct OutputWriterContext* ctx);
     ...
     */
    long long (*get_total_bytes_written)(const struct OutputWriterContext* ctx);
} OutputWriterOps;
 ...
 */
typedef struct OutputWriterContext {
    void*         private_data;         ///< Pointer to implementation-specific data (e.g., FILE* or SNDFILE*).
    OutputWriterOps api;                  ///< The set of function pointers for the selected writer implementation.
 ...
 *
 * @param ctx Pointer to the OutputWriterContext to initialize.
 ...
 */
bool output_writer_init(struct OutputWriterContext* ctx, const struct AppConfig* config);
